...
namespace CapaNegocio
{
    public class NotificacionBL
    {
        ...
        public static bool NotificarCambioEstado(int codigoUsuario, int codigoSolicitud, string nuevoEstado)
        {
            ...
            string tipo = "INFO";
            string titulo = "Cambio de Estado";
            ...
            var notificacionInternaOk = EnviarNotificacion(
                codigoUsuario,
                titulo,
        ...
        private static bool EncolarCorreoCambioEstadoIdempotente(int codigoUsuario, int codigoSolicitud, string estadoNormalizado, string titulo, string tipo)
        {
            try
            {
                ...
                var emailDestino = usuario != null ? (usuario.Email ?? string.Empty).Trim() : string.Empty;
                if (string.IsNullOrWhiteSpace(emailDestino))
                {
                    return false;
                }

                var nombreDestino = usuario != null
                    ? (!string.IsNullOrWhiteSpace(usuario.NombreCompleto) ? usuario.NombreCompleto : usuario.NombreUsuario)
                    : "Usuario";

                var eventKey = ConstruirEventKeyCambioEstado(codigoUsuario, codigoSolicitud, estadoNormalizado);
                var queueItem = new EmailQueueItem
                {
                    Para = emailDestino,
                    ParaNombre = nombreDestino,
                    Asunto = "AOCR - " + titulo,
            ...
            {
                LogBL.RegistrarError("Error al encolar correo idempotente de cambio de estado", ex.ToString(), "Notificacion");
                return false;
            }
        }

        private static string ConstruirEventKeyCambioEstado(int codigoUsuario, int codigoSolicitud, string estadoNormalizado)
        {
        ...
        {
            var color = "#1B4F72";
            if (string.Equals(tipo, "WARNING", StringComparison.OrdinalIgnoreCase))
            {
                color = "#B9770E";
            }
            else if (string.Equals(tipo, "ERROR", StringComparison.OrdinalIgnoreCase))
            {
                color = "#922B21";
            }
            else if (string.Equals(tipo, "SUCCESS", StringComparison.OrdinalIgnoreCase))
            {
                color = "#1E8449";
            }
...
<body style='font-family: Arial, sans-serif; padding: 16px;'>
  <div style='max-width: 620px; margin: 0 auto; border: 1px solid #e5e7eb; border-radius: 8px; overflow: hidden;'>
    <div style='background:{0}; color:#fff; padding: 12px 16px; font-weight: 600;'>Sistema AOCR</div>
    <div style='padding: 16px;'>
      <p>Estimado/a <strong>{1}</strong>,</p>
      <p>La solicitud <strong>#{2}</strong> cambió al estado <strong>{3}</strong>.</p>
...
</html>", color, nombreDestino ?? "Usuario", codigoSolicitud, estadoNormalizado);
        }
