...
namespace CapaPresentacion.Helpers
{
    public static class SidebarMenuBuilder
    {
        private sealed class SidebarBadgeSnapshot
        {
            ...
            public int InspectorAocrEditable { get; set; }
            public int InspectorCondicionesEditable { get; set; }
            public int InspectorDocumentosObservados { get; set; }
            public int InspectorDocumentosEnviados { get; set; }
            ...
            public int DcavPendingReview { get; set; }
            public int DcavPendingReports { get; set; }
            public int DcavPendingDocuments { get; set; }
            public int DcavPendingSignatures { get; set; }
        ...
        private static SidebarBuildContext BuildContext(ViewContext viewContext, ViewDataDictionary viewData, object model)
        {
            ...
            context.EsDirectorCertificacionesDcavRol = permission.EsDirectorCertificacionesDcavRol;
        ...
        private static SidebarBadgeSnapshot LoadBadgeSnapshot(SidebarBuildContext context)
        {
            ...
            var cachedTime = session[cacheTimeKey] as long?;
            var now = DateTime.UtcNow.Ticks;
            var cacheSeconds = context.EsFinancieroRol
                || context.EsAdministrador
                || context.EsDirectorCertificacionesDcavRol
                || context.EsDirdacRol
                || context.EsDirectorGeneralRol
                || context.EsInspectorRol
                ? 5
                : 45;
            ...
            try
            {
                if (DebeCargarContadorRol(context, "inspector"))
                {
                    ...
                    var documentosInspector = new FirmaAocrInspectorQueueService().Obtener(context.UserId, context.EsAdministrador);
                    snapshot.InspectorAocrEditable = documentosInspector.AocrEditables + documentosInspector.Observados.Count;
                    snapshot.InspectorCondicionesEditable = documentosInspector.CondicionesEditables + documentosInspector.Observados.Count;
                    snapshot.InspectorDocumentosObservados = documentosInspector.Observados.Count;
                    snapshot.InspectorDocumentosEnviados = documentosInspector.Enviados.Count;
                    Trace.TraceInformation("[SIDEBAR][AOCR_COUNT] UsuarioId=" + context.UserId + "; Cantidad=" + snapshot.InspectorAocrEditable + ";");
                    Trace.TraceInformation("[SIDEBAR][CONDICIONES_COUNT] UsuarioId=" + context.UserId + "; Cantidad=" + snapshot.InspectorCondicionesEditable + ";");
                    Trace.TraceInformation("[SIDEBAR][DOCUMENTOS_OBSERVADOS_COUNT] UsuarioId=" + context.UserId + "; Cantidad=" + snapshot.InspectorDocumentosObservados + ";");
                    Trace.TraceInformation("[SIDEBAR][DOCUMENTOS_ENVIADOS_COUNT] UsuarioId=" + context.UserId + "; Cantidad=" + snapshot.InspectorDocumentosEnviados + ";");
                }
            }
            catch (Exception ex)
            {
                ...
                snapshot.InspectorAocrEditable = 0;
                snapshot.InspectorCondicionesEditable = 0;
                snapshot.InspectorDocumentosObservados = 0;
                snapshot.InspectorDocumentosEnviados = 0;
            ...
            try
            {
                if (context.EsDirectorCertificacionesDcavRol)
                {
                    var dcavDao = new AocrDcavDAO();
                    var informes = dcavDao.ObtenerPendientesRevisionInforme();
                    var documentosCount = new AocrDcavDocumentosDAO().ContarPendientesRevisionDocumentos();
                    var firmasCondicionesCount = new FirmaInstitucionalAocrService().ContarPendientesDcav();
                    var observados = dcavDao.ObtenerObservados();
                    var legacy = dcavDao.ObtenerLegacyPendientesRevisionDcav();
                    var recuperados = new AocrDcavRevisionService().ListarPendientes().Count;
                    snapshot.DcavPendingReview = informes.Count + documentosCount;
                    snapshot.DcavPendingReports = informes.Count;
                    snapshot.DcavPendingDocuments = documentosCount;
                    snapshot.DcavPendingSignatures = firmasCondicionesCount;
                    snapshot.ExecutiveApprovalQueue = snapshot.DcavPendingReview;
                    snapshot.DirdacPendingSignatures = 0;
                    Trace.TraceInformation("[SIDEBAR_COUNTERS][DCAV] UsuarioId=" + context.UserId
                        + "; Rol=" + (context.RolActual ?? string.Empty)
                        + "; Informes=" + informes.Count
                        + "; Documentos=" + documentosCount
                        + "; FirmasCondiciones=" + firmasCondicionesCount
                        + "; Observados=" + observados.Count
                        + "; Legacy=" + legacy.Count
                        + "; Recuperados=" + recuperados
                        + "; Cantidad=" + snapshot.DcavPendingReview + ";");
                }
                else if (DebeCargarContadorRol(context, "direccion"))
                {
            ...
            {
                snapshot.DcavPendingReview = 0;
                snapshot.DcavPendingReports = 0;
                snapshot.DcavPendingDocuments = 0;
                snapshot.DcavPendingSignatures = 0;
        ...
        private static SidebarMenuGroupViewModel BuildInicioGroup(SidebarBuildContext context)
        {
            ...
            {
                pendientesController = "FirmaAocr";
                pendientesAction = "Pendientes";
        ...
        private static SidebarMenuGroupViewModel BuildFlujoGroup(SidebarBuildContext context)
        {
            ...
            var aocrCondicionesBadge = context.EsSolicitanteORT
                ...
                : (context.EsDirdacRol || context.EsCoordinadorRol || context.EsLegalRol || context.EsAdministrador
                    ...
                    : 0);
            group.Items.Add(CreateItem(context, "aocr-condiciones", "AOCR y condiciones", "Revisión, validación y emisión de documentos AOCR.", "fas fa-certificate", context.EsInspectorRol ? "InspectorDocumentosFinales" : (context.EsDirdacRol ? "FirmaAocr" : "SolicitudAOCR"), context.EsInspectorRol ? "Revision" : (context.EsDirdacRol ? "Pendientes" : (context.EsLegalRol ? "RevisarLegalizacion" : "GeneradasFirmadas")), null, aocrCondicionesBadge, "info", context.EsSolicitanteORT || context.EsInspectorRol || context.EsCoordinadorRol || context.EsLegalRol || context.EsDirdacRol || context.EsAdministrador, true, string.Empty, new[] { context.EsInspectorRol ? "Revision" : (context.EsDirdacRol ? "Pendientes" : (context.EsLegalRol ? "RevisarLegalizacion" : "GeneradasFirmadas")) }, null, null, string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildBandejasGroup(SidebarBuildContext context)
        {
            ...
            group.Items.Add(CreateItem(context, "listos-firma", "Listos para firma", "Documentos que llegaron a fase de validación o firma institucional.", "fas fa-signature", context.EsDirdacRol ? "FirmaAocr" : (context.EsLegalRol ? "SolicitudAOCR" : "CoordinacionJefatura"), context.EsDirdacRol ? "Pendientes" : (context.EsLegalRol ? "RevisarLegalizacion" : "DashboardInspeccion"), null, context.EsDirdacRol ? context.Badges.DirdacPendingSignatures : context.Badges.ExecutiveApprovalQueue, "danger", context.EsCoordinadorRol || context.EsLegalRol || context.EsDirdacRol || context.EsAdministrador, true, string.Empty, new[] { context.EsDirdacRol ? "Pendientes" : (context.EsLegalRol ? "RevisarLegalizacion" : "DashboardInspeccion") }, null, null, string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildFirmasGroup(SidebarBuildContext context)
        {
            ...
            group.Items.Add(CreateItem(context, "aocr-firma", "Firma institucional AOCR", "Documentos AOCR listos para validacion y firma institucional.", "fas fa-stamp", "FirmaAocr", "Pendientes", null, context.Badges.DirdacPendingSignatures, "warning", context.EsDirdacRol || context.EsDirectorGeneralRol || context.EsAdministrador, true, string.Empty, new[] { "Pendientes", "Index" }, null, null, string.Empty));
            group.Items.Add(CreateItem(context, "condiciones-firma", "Condiciones dentro de firma AOCR", "Condiciones y limitaciones gestionadas desde la firma institucional AOCR.", "fas fa-file-contract", "FirmaAocr", "Pendientes", null, context.Badges.DirdacPendingSignatures, "warning", context.EsDirdacRol || context.EsDirectorGeneralRol || context.EsAdministrador, true, string.Empty, new[] { "Pendientes", "Index" }, null, null, string.Empty));
            group.Items.Add(CreateItem(context, "legalizacion", "Legalización y firma DIRDAC", "Revisión jurídica, emisión y cierre formal del documento final.", "fas fa-gavel", context.EsLegalRol ? "SolicitudAOCR" : "FirmaAocr", context.EsLegalRol ? "RevisarLegalizacion" : "Pendientes", null, context.Badges.DirdacPendingSignatures, "info", context.EsLegalRol || context.EsDirdacRol || context.EsAdministrador, true, string.Empty, new[] { context.EsLegalRol ? "RevisarLegalizacion" : "Pendientes" }, null, null, string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildInicioMenuGroup(SidebarBuildContext context)
        {
            ...
            else if (context.EsDirdacRol || context.EsDirectorGeneralRol)
            {
                ...
                workController = "FirmaAocr";
                workAction = "Pendientes";
                workDescription = "Bandeja de AOCR y Condiciones pendientes de firma institucional.";
                workBadge = context.Badges.DirdacPendingSignatures;
                pendingController = "FirmaAocr";
                pendingAction = "Pendientes";
                pendingDescription = "AOCR y Condiciones pendientes de firma o devolución.";
        ...
        private static SidebarMenuGroupViewModel BuildSolicitudMenuGroup(SidebarBuildContext context)
        {
            ...
            group.Items.Add(CreateItem(context, "sol-rt-nueva", context.PuedeAbrirFormularioEmisionRt ? "Nueva / continuar solicitud" : (DebeMostrarSeguimientoSolicitudRt(context) ? "Ver seguimiento AOCR" : "Nueva / continuar solicitud"), context.PuedeAbrirFormularioEmisionRt ? "Inicia o retoma la solicitud principal de Emision / Renovacion / Modificacion AOCR." : (DebeMostrarSeguimientoSolicitudRt(context) ? "Consulte el estado del trámite y continúe el flujo desde el detalle." : "Inicia o retoma la solicitud principal de Emision / Renovacion / Modificacion AOCR."), "fas fa-play-circle", "SolicitudAOCR", ResolverAccionSolicitudRtSidebar(context, "FormularioEmisionAOCR"), ResolverRouteValuesSolicitudRtSidebar(context, 1), context.Badges.RtActiveRequests, "info", context.EsSolicitanteORT, context.TieneAccesoSolicitudRt || context.PuedeAbrirFormularioEmisionRt, context.TieneAccesoSolicitudRt || context.PuedeAbrirFormularioEmisionRt ? string.Empty : context.MensajeBloqueoRtSidebar, ResolverActiveActionsSolicitudRtSidebar(context, "FormularioEmisionAOCR", "Index"), "tipoSolicitud", "1", string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildInspeccionesMenuGroup(SidebarBuildContext context)
        {
            ...
            group.Items.Add(CreateItem(context, "insp-mis", "Mis tramites", "Bandeja principal de tramites asignados al inspector.", "fas fa-plane", "Inspeccion", "Index", null, context.Badges.InspectorTotalInspections, "neutral", context.EsInspectorRol, true, string.Empty, new[] { "Index", "Detalle" }, null, null, string.Empty));
        ...
        {
            var group = NewGroup("informe-main", "Inspeccion e Informe Tecnico", "fas fa-file-lines", "LV/EAE, elaboración del informe técnico y seguimiento de revisión o aprobación.", "secondary");
            ...
            group.Items.Add(CreateItem(context, "inf-crear", "Crear Inspeccion e Informe Tecnico", "Ingreso general al módulo de informes técnicos del proceso.", "fas fa-file-circle-plus", "Informe", "Index", null, 0, "info", context.EsInspectorRol, true, "Disponible después de firmar la LV cuando la inspección corresponda.", new[] { "Index" }, null, null, string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildAocrCondicionesMenuGroup(SidebarBuildContext context)
        {
            ...
            group.Items.Add(CreateItem(context, "aocr-ins-genera", "AOCR Y CONDICIONES", "Sección exclusiva para revisar, guardar, previsualizar y generar ambos documentos finales.", "fas fa-file-signature", "InspectorDocumentosFinales", "Revision", null, context.Badges.InspectorAocrEditable, "info", context.EsInspectorRol, true, string.Empty, new[] { "Revision", "Detalle" }, null, null, string.Empty));
            group.Items.Add(CreateItem(context, "aocr-ins-devueltas", "Observaciones DCAV", "Documentos finales observados para corrección por el Inspector asignado.", "fas fa-rotate-left", "InspectorDocumentosFinales", "Revision", null, context.Badges.InspectorDocumentosObservados, "danger", context.EsInspectorRol, true, string.Empty, new[] { "Revision", "Detalle" }, null, null, string.Empty));
            ...
            group.Items.Add(CreateItem(context, "aocr-dcav-revision", "Informes técnicos", "Informes técnicos firmados pendientes de decisión DCAV.", "fas fa-clipboard-check", "AocrDcav", "Revision", new { fase = "informe" }, context.Badges.DcavPendingReports, "warning", context.EsDirectorCertificacionesDcavRol || context.EsAdministrador, true, string.Empty, new[] { "Revision", "Detalle" }, "fase", "informe", string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildFirmasAprobacionesMenuGroup(SidebarBuildContext context)
        {
            ...
            var visibleDcav = context.EsDirectorCertificacionesDcavRol || context.EsAdministrador;
            ...
            group.Items.Add(CreateItem(context, "firm-dcav-revision", "AOCR y Condiciones", "AOCR y Condiciones enviados por el Inspector antes de firma del Director General.", "fas fa-clipboard-check", "AocrDcav", "RevisionDocumentos", null, context.Badges.DcavPendingDocuments, "warning", visibleDcav, true, string.Empty, new[] { "RevisionDocumentos", "DetalleDocumentos", "HistorialDocumentos" }, string.Empty, string.Empty, string.Empty));
            group.Items.Add(CreateItem(context, "firm-dcav-condiciones", "Firmar Condiciones", "Condiciones y Limitaciones aprobadas pendientes de firma DCAV.", "fas fa-file-signature", "FirmaInstitucionalAocr", "Pendientes", null, context.Badges.DcavPendingSignatures, "danger", visibleDcav, true, string.Empty, new[] { "Pendientes", "Detalle" }, null, null, string.Empty));
            group.Items.Add(CreateItem(context, "firm-dir-aocr", "Firma institucional AOCR", "AOCR y Condiciones aprobados por DCAV disponibles para consulta institucional.", "fas fa-signature", "FirmaInstitucionalAocr", "Pendientes", null, context.Badges.DirdacPendingSignatures, "info", visibleDireccion, true, string.Empty, new[] { "Pendientes", "Detalle" }, null, null, string.Empty));
        ...
        private static SidebarMenuGroupViewModel BuildHistorialMenuGroup(SidebarBuildContext context)
        {
            ...
            else if (context.EsDirectorCertificacionesDcavRol)
            {
                historyController = "AocrDcav";
                historyAction = "Revision";
                historyDescription = "Historial operativo de expedientes revisados u observados por DCAV.";
                historyBadge = context.Badges.ExecutiveApprovalQueue;
            }
            else if (context.EsAdministrador)
            {
                historyController = "SolicitudAOCR";
        ...
        private static IList<SidebarMenuItemViewModel> BuildQuickActions(SidebarBuildContext context)
        {
            ...
            if (context.EsInspectorRol)
            {
                ...
                actions.Add(CreateQuickAction(context, "qa-inspecciones", "Mis tramites", "fas fa-plane-departure", "Inspeccion", "Index", null, context.Badges.InspectorTotalInspections, "neutral", true, true, string.Empty));
                actions.Add(CreateQuickAction(context, "qa-informe", "Inspeccion e Informe Tecnico", "fas fa-file-lines", "Informe", "Index", null, 0, "neutral", true, true, string.Empty));
                actions.Add(CreateQuickAction(context, "qa-aocr-corregir", "Revisar AOCR y Condiciones", "fas fa-certificate", "InspectorDocumentosFinales", "Revision", null, context.Badges.InspectorAocrEditable + context.Badges.InspectorDocumentosObservados, "warning", true, true, string.Empty));
            }
            ...
            if (context.EsDirectorCertificacionesDcavRol)
            {
                actions.Add(CreateQuickAction(context, "qa-dcav-revision", "Revisión DCAV", "fas fa-clipboard-check", "AocrDcav", "Revision", null, context.Badges.DcavPendingReview, "warning", true, true, string.Empty));
                actions.Add(CreateQuickAction(context, "qa-dcav-firma-condiciones", "Firmar Condiciones", "fas fa-file-signature", "FirmaInstitucionalAocr", "Pendientes", null, context.Badges.DcavPendingSignatures, "danger", true, true, string.Empty));
            }

            if (context.EsDirdacRol || context.EsDirectorGeneralRol)
            {
                actions.Add(CreateQuickAction(context, "qa-firma", "Firma institucional AOCR", "fas fa-signature", "FirmaInstitucionalAocr", "Pendientes", null, context.Badges.DirdacPendingSignatures, "danger", true, true, string.Empty));
