$(function () {
    var applicationPath = (window.AocrContext && window.AocrContext.applicationPath) || '/';
    if (applicationPath.charAt(applicationPath.length - 1) !== '/') {
        applicationPath += '/';
    }

    function aocrUrl(path) {
        return applicationPath + String(path || '').replace(/^\/+/, '');
    }
    ...
    $('[data-aocr-recorrido-solicitud]').each(function () {
        ...
        if (id) {
            ...
            $.get(aocrUrl('RecorridoTramite/Ver'), { solicitudId: id }, function (html) {
                $div.html(html);
    ...
    $('[data-aocr-recorrido-orden]').each(function () {
        ...
        if (id) {
            $div.html('<div class="p-3 text-center text-muted"><i class="fas fa-spinner fa-spin fa-2x mb-2"></i><br/>Cargando recorrido del trámite...</div>');
            $.get(aocrUrl('RecorridoTramite/VerPorOrden'), { ordenId: id }, function (html) {
                $div.html(html);
                initFilterHandlers($div);
            }).fail(function() {
                $div.html('<div class="alert alert-danger m-2">Error al cargar el recorrido del trámite.</div>');
    ...
    $('[data-aocr-recorrido-inspeccion]').each(function () {
        ...
        if (id) {
            $div.html('<div class="p-3 text-center text-muted"><i class="fas fa-spinner fa-spin fa-2x mb-2"></i><br/>Cargando recorrido del trámite...</div>');
            $.get(aocrUrl('RecorridoTramite/VerPorInspeccion'), { inspeccionId: id }, function (html) {
                $div.html(html);
                initFilterHandlers($div);
            }).fail(function() {
                $div.html('<div class="alert alert-danger m-2">Error al cargar el recorrido del trámite.</div>');
    ...
    $('[data-aocr-recorrido-informe]').each(function () {
        ...
        if (id) {
            $div.html('<div class="p-3 text-center text-muted"><i class="fas fa-spinner fa-spin fa-2x mb-2"></i><br/>Cargando recorrido del trámite...</div>');
            $.get(aocrUrl('RecorridoTramite/VerPorInforme'), { informeId: id }, function (html) {
                $div.html(html);
                initFilterHandlers($div);
            }).fail(function() {
                $div.html('<div class="alert alert-danger m-2">Error al cargar el recorrido del trámite.</div>');
    ...
    $(document).on('click', '.btn-ver-recorrido', function() {
        ...
        if (ordenId && $body.length) {
            ...
            $.get(aocrUrl('RecorridoTramite/VerPorOrden'), { ordenId: ordenId }, function (html) {
                $body.html(html);
