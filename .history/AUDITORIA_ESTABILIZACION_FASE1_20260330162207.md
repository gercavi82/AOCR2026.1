...
2. El modulo de observaciones/no conformidades sigue con riesgo funcional real por nombre de columna inconsistente.
   - Evidencia principal:
     - `CapaDatos/DAOs/ObservacionDAO.cs`
   - Sintoma observado:
     - referencias a `codigoobservacion` en consultas y mapeo.
   - Impacto:
     - el dashboard requiere degradacion controlada para no caer completo.
   - Recomendacion:
     - corregir el DAO contra el esquema real de BD y validar en runtime.
...
- Correccion estructural del DAO de observaciones.
...
1. Corregir `ObservacionDAO` contra el esquema real y eliminar la degradacion temporal del dashboard.
