...
try {
    ...
    Write-Host "Verificando tablas relacionadas con órdenes:" -ForegroundColor Cyan
    
    $query = "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' AND table_name LIKE '%orden%' ORDER BY table_name;"
    ...
    Write-Host "Verificando tablas específicas:" -ForegroundColor Cyan
    ...
    foreach ($tableName in $tablesToCheck) {
        ...
        if ($exists) {
            ...
            $structQuery = "SELECT column_name FROM information_schema.columns WHERE table_name = '$tableName' ORDER BY ordinal_position;"
            $cmd = New-Object Npgsql.NpgsqlCommand($structQuery, $conn)
            $reader = $cmd.ExecuteReader()
            
            Write-Host "Columnas principales:" -ForegroundColor Cyan
            while ($reader.Read()) {
