...
<body>
    ...
    public class PdfGenerationException : Exceptionendregion
    {    }
        public PdfGenerationException(string message, Exception inner = null) : base(message, inner) { }
    }region Excepciones

    #endregionidationException : Exception
    {
    #region Registro de Generacióne) : base(message) { }

    public class PdfGeneracionRegistro
    {ublic class PdfGenerationException : Exception
        public int Id { get; set; }    {
        public string TipoDocumento { get; set; }e, Exception inner = null) : base(message, inner) { }
        public int EntidadId { get; set; }
        public string NumeroReferencia { get; set; }
        public DateTime FechaInicio { get; set; }endregion
        public DateTime? FechaFin { get; set; }
        public bool Exitoso { get; set; }gistro de Generación
        public string Error { get; set; }
        public int Intentos { get; set; }stro
        public long TamanoBytes { get; set; }    {
    }
   public string TipoDocumento { get; set; }
    public interface IPdfRegistroService set; }
    {; }
        Task RegistrarAsync(PdfGeneracionRegistro registro); set; }
    }

    public class PdfRegistroService : BaseDAO, IPdfRegistroService
    {
        public PdfRegistroService(string connectionString) : base(connectionString) { }t; }

        public async Task RegistrarAsync(PdfGeneracionRegistro registro)
        {ublic interface IPdfRegistroService
            const string sql = @"    {
                INSERT INTO pdf_generaciones (nRegistro registro);
                    tipo_documento, entidad_id, numero_referencia,
                    fecha_inicio, fecha_fin, exitoso, error, intentos, tamano_bytes
                ) VALUES (ublic class PdfRegistroService : BaseDAO, IPdfRegistroService
            ...
            {        {
                ...
                {       INSERT INTO pdf_generaciones (
                    AddParameter(cmd, "@tipo", registro.TipoDocumento);o, entidad_id, numero_referencia,
                    AddParameter(cmd, "@entidad_id", registro.EntidadId);xitoso, error, intentos, tamano_bytes
                    AddParameter(cmd, "@numero", registro.NumeroReferencia ?? (object)DBNull.Value);
                    AddParameter(cmd, "@inicio", registro.FechaInicio);intentos, @tamano
                    AddParameter(cmd, "@fin", registro.FechaFin ?? (object)DBNull.Value);
                    AddParameter(cmd, "@exitoso", registro.Exitoso);
                    AddParameter(cmd, "@error", registro.Error ?? (object)DBNull.Value);WithConnection(conn =>
                    AddParameter(cmd, "@intentos", registro.Intentos);            {
                    AddParameter(cmd, "@tamano", registro.TamanoBytes);, cmd =>
                });   {
            });gistro.TipoDocumento);
        }   AddParameter(cmd, "@entidad_id", registro.EntidadId);
    }cia ?? (object)DBNull.Value);
