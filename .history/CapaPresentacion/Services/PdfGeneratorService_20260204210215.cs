...
namespace CapaPresentacion.Services
{
    ...
    public class PdfGeneratorService : IPdfGeneratorService
    {
        ...
        private string GenerarHtmlOrden(OrdenRecaudacion orden)
        {
            ...
            // Obtener configuración de PDF desde parámetros
            var configPdf = _parametroDAO.ObtenerConfiguracionPDF();
            
            var tituloPrincipal = configPdf.ContainsKey("PDF_TITULO_PRINCIPAL") 
                ? configPdf["PDF_TITULO_PRINCIPAL"] 
                : "DIRECCIÓN GENERAL DE AVIACIÓN CIVIL";
            
            var subtituloOrden = configPdf.ContainsKey("PDF_SUBTITULO_ORDEN") 
                ? configPdf["PDF_SUBTITULO_ORDEN"] 
                : "ORDEN DE RECAUDACIÓN";
            
            var footerLinea1 = configPdf.ContainsKey("PDF_FOOTER_LINEA_1") 
                ? configPdf["PDF_FOOTER_LINEA_1"] 
                : "Dirección General de Aviación Civil del Ecuador";
            
            var footerLinea2 = configPdf.ContainsKey("PDF_FOOTER_LINEA_2") 
                ? configPdf["PDF_FOOTER_LINEA_2"] 
                : "Sistema AOCR - Autorización de Explotador de Servicios Aéreos";
...
<body>
    ...
    <div class='section'>
        ...
        <table>
            ...
            <tr><td class='label'>Fecha de Creación:</td><td>{2:dd/MM/yyyy HH:mm}</td></tr>
            <tr><td class='label'>Estado:</td><td>{3}</td></tr>
        </table>
    </div>
    ...
    <div class='section'>
        ...
        <table>
            <tr><td class='label'>Nombre/Razón Social:</td><td>{4}</td></tr>
            <tr><td class='label'>RUC/CI:</td><td>{5}</td></tr>
            <tr><td class='label'>Correo Electrónico:</td><td>{6}</td></tr>
        </table>
    </div>
    ...
    <div class='section'>
        ...
        <table>
            <tr><td class='label'>Subtotal:</td><td style='text-align: right;'>$ {7:N2}</td></tr>
            <tr><td class='label'>IVA:</td><td style='text-align: right;'>$ {8:N2}</td></tr>
            <tr><td class='label' style='font-size: 14px;'>TOTAL A PAGAR:</td><td style='text-align: right; font-size: 14px; font-weight: bold;'>$ {9:N2}</td></tr>
        </table>
    </div>
    ...
    <div class='footer'>
        <p>{12}</p>
        <p>{13}</p>
        <p>Generado el: {14:dd/MM/yyyy HH:mm}</p>
    </div>
</body>
</html>",
                numeroOrden,                    // {0}
                tituloPrincipal,               // {1}
                orden.FechaCreacion,           // {2}
                orden.Estado ?? "PENDIENTE",   // {3}
                nombreContribuyente,           // {4}
                rucContribuyente,              // {5}
                emailContribuyente,            // {6}
                orden.Subtotal,                // {7}
                orden.Iva,                     // {8}
                orden.Total,                   // {9}
                subtituloOrden,                // {10}
                string.IsNullOrEmpty(observaciones) ? "" :  // {11}
