...
namespace CapaPresentacion.Controllers
{
    ...
    public class InspeccionController : Controller
    {
        ...
        [ValidateAntiForgeryToken]
        [ValidateInput(false)]
        public ActionResult GuardarInformeTecnico(int id, bool finalizar = false)
        {
            try
            {
                if (id <= 0)
                ...
                // Cuando finalizar no llega como parámetro, se toma desde el form.
                if (!finalizar)
                {
                    var finalizarRaw = form?["finalizar"];
