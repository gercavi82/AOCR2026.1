...
namespace CapaPresentacion.Controllers
{
    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public ActionResult Index(string estado)
        {
            ...
            var ordenes = _dao.ListarPorUsuario(idUsuario, estado) ?? new List<OrdenRecaudacion>();
        ...
        [Authorize(Roles = "Solicitante,Administrador")]
        [ValidateAntiForgeryToken]
        public ActionResult Editar(OrdenRecaudacion model)
        {
            int idUsuario = GetUserId();
            if (idUsuario <= 0) return RedirectToAction("Login", "Account");
        ...
        private void CargarConceptosNueva(CapaPresentacion.Models.OrdenRecaudacionNuevaVM model)
        {
            ...
            var conceptos = _conceptoDao.ObtenerConceptos(true) ?? new List<CapaModelo.ConceptoModel>();
        ...
        {
            var conceptos = new List<CapaModelo.ConceptoModel>
            {
                new CapaModelo.ConceptoModel { Codigo = "EMI_AOCR", Nombre = "Emisión AOCR", TipoCalculo = "FIJO", ValorBase = 3300m, PorcentajeAdmin = 0m, Activo = true, Orden = 1, Descripcion = "Emisión AOCR", PorEstacion = false, PorDia = false, EsViatico = false },
                new CapaModelo.ConceptoModel { Codigo = "REN_AOCR", Nombre = "Renovación AOCR", TipoCalculo = "FIJO", ValorBase = 3300m, PorcentajeAdmin = 0m, Activo = true, Orden = 2, Descripcion = "Renovación AOCR", PorEstacion = false, PorDia = false, EsViatico = false },
                new CapaModelo.ConceptoModel { Codigo = "MOD_AOCR_INC", Nombre = "Modificación AOCR (Inclusión aeronaves distinto modelo y tipo)", TipoCalculo = "FIJO", ValorBase = 1600m, PorcentajeAdmin = 0m, Activo = true, Orden = 3, Descripcion = "Modificación AOCR (Inclusión aeronaves distinto modelo y tipo)", PorEstacion = false, PorDia = false, EsViatico = false },
                new CapaModelo.ConceptoModel { Codigo = "MOD_AOCR_SIN_INC", Nombre = "Modificación AOCR (Que no implique incremento de aeronaves)", TipoCalculo = "FIJO", ValorBase = 80m, PorcentajeAdmin = 0m, Activo = true, Orden = 4, Descripcion = "Modificación AOCR (Que no implique incremento de aeronaves)", PorEstacion = false, PorDia = false, EsViatico = false },
                new CapaModelo.ConceptoModel { Codigo = "INSPECCION_EXT", Nombre = "Inspección requerida por el Operador Aéreo Extranjero", TipoCalculo = "POR_ESTACION", ValorBase = 500m, PorcentajeAdmin = 0m, Activo = true, Orden = 5, Descripcion = "Inspección requerida por el Operador Aéreo Extranjero (por estación)", PorEstacion = true, PorDia = false, EsViatico = false },
                new CapaModelo.ConceptoModel { Codigo = "VIATICOS_INSPECTOR", Nombre = "Viáticos a Sres. Inspectores", TipoCalculo = "POR_DIA", ValorBase = 80m, PorcentajeAdmin = 8m, Activo = true, Orden = 6, Descripcion = "Viáticos por día (más 8% de gastos administrativos)", PorEstacion = false, PorDia = true, EsViatico = true }
            };
        ...
        private async Task CargarViewBagsParaNueva()
        {
            try
            {
                ViewBag.Conceptos = _conceptoDao.ObtenerConceptos(true) ?? new List<CapaModelo.ConceptoModel>();
            }
            catch
            {
        ...
        private void EnviarNotificacionAFinanciero(OrdenRecaudacion orden, CapaModelo.PagoModel pago, string emailFinanciero, string comprobanteRuta)
        {
            try
            {
