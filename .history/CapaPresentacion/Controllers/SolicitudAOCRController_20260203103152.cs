        ...
        public JsonResult ObtenerMisSolicitudes()
        {
            try
            {
                ...
                var resultado = solicitudes.Select(s => new
                {
                    ...
                    tipo = s.TipoSolicitud ?? "EMISIÓN",
                    comp = s.NombreCompania ?? "Sin Compañía",
                    insp = s.NombreInspector ?? "Sin Asignar",
                    st = ObtenerEstadoLegible(s.EstadoSolicitud),
                    cat = ObtenerCategoria(s.EstadoSolicitud),
                    viat = s.MontoViaticos ?? 0.00m
                }).ToList();
