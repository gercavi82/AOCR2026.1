    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public ActionResult DescargarPdf(int id)
        {
            ...
            try
            {
                ...
                CapaDatos.Entidades.Solicitud solicitud = null;
                if (ordenModel.CodigoSolicitud.HasValue && ordenModel.CodigoSolicitud.Value > 0)
                {
                    var solicitudDAO = new CapaDatos.DAOs.SolicitudDAO();
                    solicitud = solicitudDAO.ObtenerPorId(ordenModel.CodigoSolicitud.Value);
                }

                // Si no se encuentra solicitud, buscar órdenes relacionadas usando los nuevos métodos
                if (solicitud == null && ordenModel.CodigoSolicitud.HasValue)
                {
                    var ordenesRelacionadas = _dao.ObtenerPorCodigoSolicitud(ordenModel.CodigoSolicitud.Value);
                ...
                // Crear modelo PDF con valores dinámicos reales de la base de datos
                var pdfModel = new CapaPresentacion.Models.ViewModels.OrdenRecaudacionPDFModel
                {
                    ...
                    LugarEmision = ordenModel.LugarEmision ?? "Quito",
                    NombreCompania = solicitud?.EmpresaRazonSocial ?? ordenModel.NombreContribuyente ?? ordenModel.Compania ?? "Empresa no especificada",
                    Ruc = solicitud?.EmpresaRuc ?? ordenModel.Ruc ?? "RUC no especificado",
                    Email = solicitud?.CorreoContacto ?? ordenModel.Email ?? "correo@empresa.com",
                    Telefono = solicitud?.TelefonoContacto ?? ordenModel.Telefono ?? "Teléfono no especificado",
                    ConceptoPrincipal = "Inspección y Certificación AOCR",
                    ValorBase = ordenModel.Subtotal != 0 ? ordenModel.Subtotal : ordenModel.Total,
                    Estaciones = solicitud?.NumeroEstaciones ?? 1,
                    Dias = solicitud?.DiasEstimados ?? 1,
                    Referencia = $"Orden de Recaudación {ordenModel.NumeroOrden}"
                };
