...
namespace CapaPresentacion.Controllers
{
    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public ActionResult DiagnosticoTarifas()
        {
            ...
            diagnostico.AppendLine("<tr><th>Clave</th><th>Valor BD</th><th>Valor Parseado</th><th>Estado</th></tr>");
            ...
            foreach (var claveTarifa in tarifas)
            {
                try
                {
                    ...
                    var valorBD = parametro?.Valor ?? "[NO EXISTE]";
                    var activo = parametro?.Activo ?? false;
                    
                    if (parametro == null)
                    {
                        diagnostico.AppendLine($"<tr class='warning'><td>{claveTarifa}</td><td colspan='3'>⚠ Parámetro no existe en BD</td></tr>");
                        continue;
                    }
                    
                    if (!activo)
                    {
                        diagnostico.AppendLine($"<tr class='warning'><td>{claveTarifa}</td><td>{valorBD}</td><td>-</td><td>⚠ Inactivo</td></tr>");
                        continue;
                    }
                    ...
                    var estado = parseExitoso ? $"✓ OK ({metodo})" : "✗ ERROR - No se puede parsear";
                    
                    diagnostico.AppendLine($"<tr class='{cssClass}'><td>{claveTarifa}</td><td>'{valorBD}'</td><td>{(parseExitoso ? valorParseado.ToString("0.00") : "N/A")}</td><td>{estado}</td></tr>");
                }
                catch (Exception ex)
                {
                    diagnostico.AppendLine($"<tr class='error'><td>{claveTarifa}</td><td colspan='3'>✗ Excepción: {ex.Message}</td></tr>");
                }
            }
