...
namespace CapaPresentacion.Controllers
{
    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public OrdenRecaudacionController()
        {
            ...
            try
            {
                ...
                _orchestrator = new OrdenRecaudacionOrchestrator(
                    ...
                    null,
                    null,
                    new CapaDatos.Services.EmailService(),
                    null
                );
        ...
        // Diagnóstico de tarifas - Ver valores actuales y parseo
        [Authorize(Roles = "Administrador")]
        public ActionResult DiagnosticoTarifas()
        {
            var diagnostico = new System.Text.StringBuilder();
            diagnostico.AppendLine("<html><head><meta charset='utf-8'><style>");
            diagnostico.AppendLine("body { font-family: Consolas, monospace; padding: 20px; }");
            diagnostico.AppendLine("table { border-collapse: collapse; width: 100%; margin-top: 20px; }");
            diagnostico.AppendLine("th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }");
            diagnostico.AppendLine("th { background-color: #4CAF50; color: white; }");
            diagnostico.AppendLine(".error { background-color: #ffcccc; }");
            diagnostico.AppendLine(".success { background-color: #ccffcc; }");
            diagnostico.AppendLine(".warning { background-color: #ffffcc; }");
            diagnostico.AppendLine("pre { background: #f4f4f4; padding: 10px; border-radius: 4px; }");
            diagnostico.AppendLine("</style></head><body>");
            diagnostico.AppendLine("<h1>Diagnóstico de Tarifas</h1>");
            diagnostico.AppendLine($"<p><strong>Fecha/Hora:</strong> {DateTime.Now:yyyy-MM-dd HH:mm:ss}</p>");
            ...
            // Consulta SQL directa para obtener valores RAW
            ...
            try
            {
                ...
                try
                {
                    // Obtener valor RAW de la consulta directa
                    string valorBD = valoresDB.ContainsKey(claveTarifa) ? valoresDB[claveTarifa] : "[NO ENCONTRADO]";
                    
                    if (valorBD == "[NULL]" || valorBD == "[NO ENCONTRADO]")
                    {
                        diagnostico.AppendLine($"<tr class='warning'><td>{claveTarifa}</td><td colspan='5'>⚠ Parámetro no existe o valor NULL en BD</td></tr>");
                        continue;
                    }
                    ...
                    var hexValues = string.Join(" ", bytes.Take(30).Select(b => b.ToString("X2")));
                    
                    // Intentar parsear
                    var valorLimpio = valorBD.Trim()
                        .Replace("$", "")
                        .Replace("USD", "")
                        .Replace(" ", "")
                        .Replace("_", "");
                    
                    decimal valorParseado = 0m;
                    var parseExitoso = false;
                    var metodo = "";
                    
                    // Intento 1: InvariantCulture
                    if (decimal.TryParse(valorLimpio, System.Globalization.NumberStyles.Any,
                        System.Globalization.CultureInfo.InvariantCulture, out valorParseado))
                    {
                        parseExitoso = true;
                        metodo = "InvariantCulture";
                    }
                    // Intento 2: Cultura española
                    else if (decimal.TryParse(valorLimpio, System.Globalization.NumberStyles.Any,
                        new System.Globalization.CultureInfo("es-ES"), out valorParseado))
                    {
                        parseExitoso = true;
                        metodo = "es-ES";
                    }
                    // Intento 3: Reemplazar coma por punto
                    else
                    {
                        var valorConPunto = valorLimpio.Replace(",", ".");
                        if (decimal.TryParse(valorConPunto, System.Globalization.NumberStyles.Any,
                            System.Globalization.CultureInfo.InvariantCulture, out valorParseado))
                        {
                            parseExitoso = true;
                            metodo = "Coma→Punto";
                    ...
                    var cssClass = parseExitoso ? "success" : "error";
                    var estado = parseExitoso ? $"✓ OK ({metodo})" : $"✗ ERROR - No parseable<br/><small>Limpio: '{valorLimpio}'</small>";
                    
                    diagnostico.AppendLine($"<tr class='{cssClass}'><td>{claveTarifa}</td><td><code>'{valorBD}'</code></td><td>{longitud}</td><td style='font-size:10px;'>{hexValues}</td><td>{(parseExitoso ? valorParseado.ToString("0.00") : "N/A")}</td><td>{estado}</td></tr>");
                }
                catch (Exception ex)
                {
                    diagnostico.AppendLine($"<tr class='error'><td>{claveTarifa}</td><td colspan='5'>✗ Excepción: {ex.Message}<br/><small>{ex.GetType().Name
                        metodo = "es-ES";
                    }
                    // Intento 3: Reemplazar coma por punto
                    else
                    ...
                    diagnostico.AppendLine($"<tr class='{cssClass}'><td>{claveTarifa}</td><td>'{valorBD}'<br/><small>Hex: {hexValues}</small></td><td>{tipoBD}</td><td>{(parseExitoso ? valorParseado.ToString("0.00") : "N/A")}</td><td>{estado}</td></tr>");
                }
                catch (Exception ex)
                {
                    var errorMsg = !string.IsNullOrEmpty(errorDetalle) ? errorDetalle : ex.Message;
                    var stackPreview = ex.StackTrace != null ? ex.StackTrace.Split('\n').FirstOrDefault() : "";
                    diagnostico.AppendLine($"<tr class='error'><td>{claveTarifa}</td><td>'{valorBD}'</td><td>{tipoBD}</td><td colspan='2'>✗ Excepción: {errorMsg}<br/><small>{ex.GetType().Name}</small><br/><small style='color:#666;'>{stackPreview}</small></td></tr>");
                }
            }
            
            diagnostico.AppendLine("</table>");
            diagnostico.AppendLine("<h3>Recomendación</h3>");
            diagnostico.AppendLine("<p>Los valores en la BD deben tener formato numérico: <code>1234.56</code> o <code>1234,56</code></p>");
            diagnostico.AppendLine("<p>Evitar: símbolos de moneda ($), separadores de miles con punto (1.234,56), espacios, guiones bajos.</p>");
            diagnostico.AppendLine("</body></html>");
            
            return Content(diagnostico.ToString(), "text/html");
        }
            diagnostico.AppendLine("<h3>Recomendación</h3>");
            diagnostico.AppendLine("<p>Los valores en la BD deben tener formato numérico: <code>1234.56</code> o <code>1234,56</code></p>");
            diagnostico.AppendLine("<p>Evitar: símbolos de moneda ($), separadores de miles con punto (1.234,56), espacios, guiones bajos.</p>");
            
            diagnostico.AppendLine("<h3>Comandos SQL para corregir</h3>");
            diagnostico.AppendLine("<pre>");
            diagnostico.AppendLine("-- Ejecutar estos comandos en pgAdmin o psql:");
            foreach (var clave in tarifas)
            {
                var valorDefault = "0";
                if (clave.Contains("EMI")) valorDefault = "3300";
                else if (clave.Contains("REN")) valorDefault = "3300";
                else if (clave.Contains("MOD") && clave.Contains("INC")) valorDefault = "1600";
                else if (clave.Contains("MOD") && clave.Contains("SIN")) valorDefault = "80";
                else if (clave.Contains("INSPECCION")) valorDefault = "500";
                else if (clave.Contains("VIATICOS")) valorDefault = "100";
                else if (clave.Contains("PORCENTAJE")) valorDefault = "10";
                
                diagnostico.AppendLine($"UPDATE aocr_tbparametro SET valor = '{valorDefault}', updatedat = NOW() WHERE clave = '{clave}' AND deletedat IS NULL;");
            }
            diagnostico.AppendLine("</pre>");
