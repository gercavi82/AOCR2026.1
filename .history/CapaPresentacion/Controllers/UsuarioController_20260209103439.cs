        ...
        public JsonResult Crear()
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(correo) || string.IsNullOrWhiteSpace(identificacion) ||
                    ...
                    string.IsNullOrWhiteSpace(apellidos) || string.IsNullOrWhiteSpace(empresaCodigo))
                {
                    return Json(new { success = false, message = "Todos los campos obligatorios deben completarse" });
                }

                // 3. PROCESAR DOCUMENTO LEGAL
                var documento = Request.Files["DocumentoRepresentante"];
                string rutaDocumento = null;

                if (documento == null || documento.ContentLength == 0)
                {
                    return Json(new { success = false, message = "Debe adjuntar el documento legal" });
                }

                // Validar extensión y tamaño
                if (!documento.FileName.ToLower().EndsWith(".pdf"))
                {
                    return Json(new { success = false, message = "Solo se permiten archivos PDF" });
                }

                if (documento.ContentLength > 2097152) // 2MB
                {
                    return Json(new { success = false, message = "El archivo no debe superar los 2MB" });
                }

                rutaDocumento = GuardarArchivoRepresentante(documento, identificacion, 0);
                
                if (string.IsNullOrEmpty(rutaDocumento))
                {
                    return Json(new { success = false, message = "Error al guardar el documento legal" });
                }
