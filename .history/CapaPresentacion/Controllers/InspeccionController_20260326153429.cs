...
namespace CapaPresentacion.Controllers
{
    ...
    public class InspeccionController : Controller
    {
        ...
        private const string ROLES_FIRMA_DIRDAC = ROL_DIRECCION + "," + ROL_DIRECTOR + "," + ROL_ADMIN;
        ...
        public ActionResult PendientesFirmaDirdac()
        {
            try
            {
                var inspecciones = _inspeccionBL.ListarTodas() ?? new List<Inspeccion>();
                var pendientes = inspecciones
                    ...
                    .Where(x => x.Informe != null
                        && x.Informe.Finalizado
                        && x.Informe.FirmadoInspector
                        && !x.Informe.FirmadoDirdac
                        && string.Equals((x.Informe.EstadoInforme ?? string.Empty).Trim(), "ENVIADO_A_DIRDAC", StringComparison.OrdinalIgnoreCase))
        ...
        private ResultadoOperacion EnviarInformeADirdacInterno(Inspeccion inspeccion, SolicitudAOCR solicitud, InspeccionInformeTecnico informe, int usuarioId)
        {
            ...
            if (informe.CorreoEnviado && string.Equals((informe.EstadoInforme ?? string.Empty).Trim(), "ENVIADO_A_DIRDAC", StringComparison.OrdinalIgnoreCase))
            {
            ...
            var resultado = _inspeccionCorreoService.NotificarEvento(inspeccion, solicitud, "PENDIENTE_FIRMA_DIRDAC", detalle);
            if (!resultado.Exitoso)
            {
                return ResultadoOperacion.Error("No fue posible notificar a DIRDAC sobre el documento pendiente de firma.");
            }

            var estadoAnterior = FirstNonEmpty(informe.EstadoInforme, "FIRMADO_INSPECTOR", "GENERADO");
            _informeDAO.MarcarEnviadoADirdac(informe.CodigoInforme, DateTime.Now, ObtenerUsuarioActual(), true, "ENVIADO_A_DIRDAC", usuarioId);
            RegistrarAuditoriaInformeDigital(
                ...
                "Documento pendiente de firma notificado a DIRDAC. IP=" + ObtenerIpCliente(),
                usuarioId,
                ObtenerUsuarioActual(),
                "ENVIO_DIRDAC");

            return ResultadoOperacion.Ok(null, "Documento pendiente de firma enviado a DIRDAC correctamente.");
        }
