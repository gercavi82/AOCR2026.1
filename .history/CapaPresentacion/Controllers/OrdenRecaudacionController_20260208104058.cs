...
namespace CapaPresentacion.Controllers
{
    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public ActionResult Nueva()
        {
            ...
            try
            {
                System.IO.File.AppendAllText(logPath, $"\n=== Nueva() INICIADO {DateTime.Now:yyyy-MM-dd HH:mm:ss.fff} ===\n");
                System.IO.File.AppendAllText(logPath, $"Usuario: {User?.Identity?.Name ?? "null"}\n");
                
                var rolesStr = "";
                var principal = User as System.Security.Principal.GenericPrincipal;
                if (principal != null) {
                    var rolesList = new List<string>();
                    if (principal.IsInRole("Solicitante")) rolesList.Add("Solicitante");
                    if (principal.IsInRole("Administrador")) rolesList.Add("Administrador");
                    if (principal.IsInRole("Operador")) rolesList.Add("Operador");
                    if (principal.IsInRole("Financiero")) rolesList.Add("Financiero");
                    rolesStr = string.Join(",", rolesList);
                }
                System.IO.File.AppendAllText(logPath, $"Roles detectados: {rolesStr}\n");
                System.IO.File.AppendAllText(logPath, $"IsAuthenticated: {User?.Identity?.IsAuthenticated ?? false}\n");
        ...
        private void CargarConceptosNueva(CapaPresentacion.Models.OrdenRecaudacionNuevaVM model)
        {
            ...
            try
            {
                System.IO.File.AppendAllText(logPath, "CargarConceptos: Llamando AsegurarConceptosBasicos...\n");
                AsegurarConceptosBasicos();

                System.IO.File.AppendAllText(logPath, "CargarConceptos: Obteniendo conceptos...\n");
                var conceptos = _conceptoDao.ObtenerConceptos(true);
                System.IO.File.AppendAllText(logPath, $"CargarConceptos: {conceptos.Count} conceptos obtenidos\n");
            ...
            {
                System.IO.File.AppendAllText(logPath, "*** ERROR Conceptos: " + ex.ToString() + "\n\n");
            ...
            try
            {
                System.IO.File.AppendAllText(logPath, "CargarConceptos: Obteniendo solicitudes...\n");
                var userId = GetUserId();
                System.IO.File.AppendAllText(logPath, $"CargarConceptos: userId = {userId}\n");
                var solicitudes = (User != null && User.IsInRole("Administrador"))
                    ? _solicitudDao.ObtenerTodos()
                    : _solicitudDao.ObtenerPorUsuario(userId);

                System.IO.File.AppendAllText(logPath, $"CargarConceptos: {solicitudes?.Count ?? 0} solicitudes obtenidas\n");
            ...
            {
                System.IO.File.AppendAllText(logPath, "*** ERROR Solicitudes: " + ex.ToString() + "\n\n");
