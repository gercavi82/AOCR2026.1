    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public ActionResult DiagnosticoTarifas()
        {
            ...
            foreach (var claveTarifa in tarifas)
            {
                ...
                {
                    diagnostico.AppendLine($"<tr class='error'><td>{claveTarifa}</td><td>'{valorBD}'</td><td>{tipoBD}</td><td colspan='2'>✗ Excepción: {ex.Message}<br/><small>{ex.GetType().Name}</small></td></tr>");
                }
            }
