...
namespace CapaPresentacion.Controllers
{
    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        public ActionResult DiagnosticoTarifas()
        {
                ...
                using (var conn = new Npgsql.NpgsqlConnection(connectionString))
                {
                    ...
                    foreach (var clave in tarifas)
                    {
                        var cmd = new Npgsql.NpgsqlCommand(
                            "SELECT valor FROM aocr_tbparametro WHERE clave = @clave AND deletedat IS NULL ORDER BY codigoparametro DESC LIMIT 1",
                            conn);
            ...
            {
                diagnostico.AppendLine($"<div class='error'><strong>Error conectando a BD:</strong> {exDB.Message}</div>");
            }
            
            diagnostico.AppendLine("<table>");
            diagnostico.AppendLine("<tr><th>Clave</th><th>Valor BD (Raw)</th><th>Longitud</th><th>Caracteres (Hex)</th><th>Valor Parseado</th><th>Estado</th></tr>");
            
            foreach (var claveTarifa in tarifas)
            {
                string valorBD = "[ERROR AL LEER]";
                string tipoBD = "Desconocido";
                string errorDetalle = "";
                
                try
                {
                    var parametro = _parametroDao.ObtenerPorClave(claveTarifa);
                    
                    if (parametro == null)
                    {
                        diagnostico.AppendLine($"<tr class='warning'><td>{claveTarifa}</td><td colspan='4'>⚠ Parámetro no existe en BD</td></tr>");
                        continue;
                    }
                    
                    // Capturar el valor como string directamente
                    try
                    {
                        valorBD = parametro.Valor ?? "[NULL]";
                        tipoBD = parametro.Valor != null ? parametro.Valor.GetType().Name : "null";
                    }
                    catch (Exception exValor)
                    {
                        valorBD = "[ERROR AL ACCEDER A VALOR]";
                        errorDetalle = $"Error en getter Valor: {exValor.Message}";
                    }
                    
                    var activo = parametro.Activo;
                    
                    // Mostrar bytes del valor para debugging
                    var bytes = System.Text.Encoding.UTF8.GetBytes(valorBD);
                    var hexValues = string.Join(" ", bytes.Take(20).Select(b => b.ToString("X2")));
                    
                    if (!activo)
                    {
                        diagnostico.AppendLine($"<tr class='warning'><td>{claveTarifa}</td><td>'{valorBD}'<br/><small>Hex: {hexValues}</small></td><td>{tipoBD}</td><td>-</td><td>⚠ Inactivo</td></tr>");
                        continue;
                    }
                    
                    if (string.IsNullOrWhiteSpace(valorBD) || valorBD == "[NULL]")
                    {
                        diagnostico.AppendLine($"<tr class='error'><td>{claveTarifa}</td><td>{valorBD}</td><td>{tipoBD}</td><td>-</td><td>✗ Valor vacío o NULL</td></tr>");
                        continue;
                    }
