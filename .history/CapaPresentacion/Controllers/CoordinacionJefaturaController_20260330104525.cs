...
namespace CapaPresentacion.Controllers
{
    ...
    public class CoordinacionJefaturaController : Controller
    {
        ...
        public ActionResult DashboardInspeccion(string compania = null, string inspector = null, string estado = null)
        {
            ...
            var noConformidades = _dashboardInspeccionDao.ObtenerNoConformidades();

            var companiasDisponibles = inspecciones.Select(x => x.Compania)
                .Concat(documentos.Select(x => x.Compania))
            ...
            var inspectoresDisponibles = inspecciones.Select(x => x.InspectorAsignado)
                .Concat(documentos.Select(x => x.InspectorAsignado))
            ...
            var estadosDisponibles = inspecciones.Select(x => x.EstadoVisual)
                .Concat(documentos.Select(x => x.EstadoDocumento))
                .Concat(firmas.Select(x => x.Estado))
                .Concat(ncData: noConformidades.Select(x => x.Estado))
                .Where(x => !string.IsNullOrWhiteSpace(x))
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .OrderBy(x => x)
                .ToList();
            ...
            {
                inspecciones = inspecciones.Where(x => ContieneTexto(x.Compania, compania)).ToList();
                documentos = documentos.Where(x => ContieneTexto(x.Compania, compania)).ToList();
            ...
            {
                inspecciones = inspecciones.Where(x => ContieneTexto(x.InspectorAsignado, inspector)).ToList();
                documentos = documentos.Where(x => ContieneTexto(x.InspectorAsignado, inspector)).ToList();
            ...
            {
                inspecciones = inspecciones.Where(x => ContieneTexto(x.EstadoVisual, estado)).ToList();
                documentos = documentos.Where(x => ContieneTexto(x.EstadoDocumento, estado)).ToList();
            ...
            var model = new DashboardInspeccionViewModel
            {
                ...
                InspeccionesEnSeguimiento = inspecciones.Select(x => new DashboardInspeccionSeguimientoItemViewModel
                {
                    CodigoInspeccion = x.CodigoInspeccion,
                    CodigoSolicitud = x.CodigoSolicitud,
                    NumeroSolicitud = x.NumeroSolicitud,
                    Compania = x.Compania,
                    InspectorAsignado = x.InspectorAsignado,
                    Estado = x.Estado,
                    EstadoVisual = x.EstadoVisual,
                    FechaAsignacion = x.FechaAsignacion,
                    UltimaActualizacion = x.UltimaActualizacion,
                    EtapaActual = x.EtapaActual,
                    UrlDetalle = urlHelper.Action("Detalle", "Inspeccion", new { id = x.CodigoInspeccion }),
                    UrlRevisar = urlHelper.Action("RevisionVerificacion", "CoordinacionJefatura"),
                    UrlContinuar = x.PuedeAsignarInspector && puedeGestionarAsignacion
                        ? urlHelper.Action("AsignarInspector", "Tecnico", new { solicitudId = x.CodigoSolicitud })
                        : (x.RequiereFirmaDirdac && puedeVerPendientesDirdac
                            ? urlHelper.Action("PendientesFirmaDirdac", "Inspeccion")
                            : (x.RequiereRevisionInstitucional
                                ? urlHelper.Action("RevisionVerificacion", "CoordinacionJefatura")
                                : urlHelper.Action("Detalle", "Inspeccion", new { id = x.CodigoInspeccion }))),
                    TextoContinuar = x.PuedeAsignarInspector && puedeGestionarAsignacion
                        ? "Asignar inspector"
                        : (x.RequiereFirmaDirdac && puedeVerPendientesDirdac ? "Pendientes de firma" : "Continuar proceso")
                }).ToList(),
                ControlDocumental = documentos.Select(x => new DashboardInspeccionDocumentoItemViewModel
                {
                    CodigoSolicitud = x.CodigoSolicitud,
                    CodigoInspeccion = x.CodigoInspeccion,
                    NumeroSolicitud = x.NumeroSolicitud,
                    ...
                    EstadoDocumento = x.EstadoDocumento,
                    FirmadoInspector = x.FirmadoInspector,
                    FirmadoDirdac = x.FirmadoDirdac,
                    FechaUltimaActualizacion = x.FechaUltimaActualizacion,
                    UrlVerDocumento = x.CodigoInspeccion.HasValue && x.TienePdf ? urlHelper.Action("VerInforme", "Inspeccion", new { id = x.CodigoInspeccion.Value }) : null,
                    UrlDescargarPdf = x.CodigoInspeccion.HasValue && x.TienePdf ? urlHelper.Action("DescargarInforme", "Inspeccion", new { id = x.CodigoInspeccion.Value }) : null,
                    UrlRevisar = x.CodigoInspeccion.HasValue
                        ? urlHelper.Action("RevisionVerificacion", "CoordinacionJefatura")
                        : urlHelper.Action("Detalle", "SolicitudAOCR", new { id = x.CodigoSolicitud }),
                    UrlFirmar = x.RequiereFirmaDirdac && puedeVerPendientesDirdac ? urlHelper.Action("PendientesFirmaDirdac", "Inspeccion") : null
                }).ToList(),
