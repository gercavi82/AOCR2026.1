...
namespace CapaPresentacion.Controllers
{
    ...
    public class CoordinacionJefaturaController : Controller
    {
        ...
        public ActionResult DashboardInspeccion(string compania = null, string inspector = null, string estado = null)
        {
            ...
            var estadosDisponibles = new[] { "ASIGNADA", "EN_PROCESO", "OBSERVADA", "SUBSANADA", "FINALIZADA" }.ToList();
            ...
            var model = new DashboardInspeccionViewModel
            {
                ...
                InspeccionesEnSeguimiento = inspecciones.Select(x => new DashboardInspeccionSeguimientoItemViewModel
                {
                    ...
                    TextoContinuar = x.PuedeAsignarInspector
                        ? "Asignar inspector"
                        : (x.RequiereFirmaDirdac ? "Pendientes de firma" : "Continuar proceso")
                }).ToList(),
                ControlDocumental = documentos.Select(x => new DashboardInspeccionDocumentoItemViewModel
                {
                    ...
                    UrlFirmar = x.RequiereFirmaDirdac ? urlHelper.Action("PendientesFirmaDirdac", "Inspeccion") : null
                }).ToList(),
                PendientesFirma = firmas.Select(x => new DashboardInspeccionFirmaItemViewModel
                {
                    ...
                    UrlAccion = string.Equals(x.FirmanteRequerido, "DIRDAC", StringComparison.OrdinalIgnoreCase)
                        ? urlHelper.Action("PendientesFirmaDirdac", "Inspeccion")
                        : urlHelper.Action("Detalle", "Inspeccion", new { id = x.CodigoInspeccion }),
                    TextoAccion = string.Equals(x.FirmanteRequerido, "DIRDAC", StringComparison.OrdinalIgnoreCase)
                        ? "Firmar"
                        : "Abrir detalle"
                }).ToList(),
