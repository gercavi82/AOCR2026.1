...
namespace CapaPresentacion.Controllers
{
    ...
    public class SyncAdminController : Controller
    {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult RunInspectoresRt()
        {
            try
            {
                var result = new InspectorMirrorPGDAO().SincronizarDesdeDb2();
                if (result.Success)
                {
                    TempData["SyncOK"] = result.Message;
                }
                else
                {
            ...
            catch (Exception ex)
            {
                ...
                {
                    Controller = "SyncAdmin",
                    Action = "RunInspectoresRt",
                    UserId = User.Identity.Name
                });
                TempData["SyncError"] = "Error ejecutando sync RT inspectores: " + ex.Message;
            }
