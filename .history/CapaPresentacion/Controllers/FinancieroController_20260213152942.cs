...
namespace CapaPresentacion.Controllers
{
    ...
    public class FinancieroController : Controller
    {
        ...
        public ActionResult ReenviarFactura(int id, string correoDestino, string observacionesCorreo)
        {
            ...
            {
                TempData["Error"] = "Sólo se puede reenviar la factura cuando la orden está en estado FACTURADA.";
                return RedirectToAction("Detalles", "OrdenRecaudacion", new { id = id });
            }

            try
            {
                // Generar PDF de la factura (si no existe se regenera)
                var adjunto = new CapaPresentacion.Services.PdfGeneratorService().GenerarOrdenRecaudacionPDF(orden);
                var nombreAdjunto = $"Factura_{orden.NumeroOrden}.pdf";

                var destinatarios = ObtenerDestinatariosFactura(orden, correoDestino);
                if (destinatarios == null || destinatarios.Count == 0)
                {
                    TempData["Error"] = "No hay destinatarios válidos para el reenvío. Ingrese un correo destino o registre el correo del contribuyente.";
                    return RedirectToAction("Detalles", "OrdenRecaudacion", new { id = id });
                }

                var asunto = $"Factura reenviada - Orden {orden.NumeroOrden}";
                var cuerpo = ConstruirCuerpoCorreoFactura(orden, observacionesCorreo);
                var emailService = new EmailService();

                var sentInfo = new System.Text.StringBuilder();
                foreach (var correo in destinatarios)
                {
                    CapaNegocio.LogBL.RegistrarInfo($"REENVIO_FACTURA | Orden={orden.NumeroOrden} | Destinatario={correo}", "FinancieroController");
                    var result = emailService.EnviarAsync(correo, "Destinatario", asunto, cuerpo, adjunto, nombreAdjunto).GetAwaiter().GetResult();
                    if (!result.Success)
                    {
                        var error = $"No se pudo reenviar factura a {correo}. {result.Error}";
                        CapaNegocio.LogBL.RegistrarError($"Fallo reenviar factura. Orden={orden.NumeroOrden}", error, "FinancieroController");
                        TempData["Error"] = error;
                        return RedirectToAction("Detalles", "OrdenRecaudacion", new { id = id });
                    }

                    var msgId = string.IsNullOrWhiteSpace(result.MessageId) ? "-" : result.MessageId;
                    sentInfo.AppendLine($"{correo} (MessageId={msgId})");
                    CapaNegocio.LogBL.RegistrarInfo($"REENVIO_FACTURA_OK | Orden={orden.NumeroOrden} | Destinatario={correo} | MessageId={msgId}", "FinancieroController");
                }

                TempData["Success"] = "Factura reenviada a: " + string.Join(", ", destinatarios) + ".";
                // Añadir detalle con MessageIds en logs (no exponer demasiado en UI)
                CapaNegocio.LogBL.RegistrarInfo($"REENVIO_FACTURA_COMPLETO | Orden={orden.NumeroOrden} | Destinatarios={string.Join(";", destinatarios)} | DetallesMessageIds={sentInfo.ToString().Trim()}", "FinancieroController");
            }
            catch (Exception ex)
            {
                CapaNegocio.LogBL.RegistrarError($"Error reenviando factura Orden={orden.NumeroOrden}", ex.ToString(), "FinancieroController");
                TempData["Error"] = "Ocurrió un error al reenviar la factura.";
            }
            return RedirectToAction("Detalles", "OrdenRecaudacion", new { id = id });
        }
