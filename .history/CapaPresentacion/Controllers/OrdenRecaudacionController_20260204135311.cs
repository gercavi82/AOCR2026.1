...
namespace CapaPresentacion.Controllers
{
    ...
    public class OrdenRecaudacionController : Controller
    {
        ...
        private string GenerarNumeroOrden()
        {
            ...
            // DEBUGGING: Verificar qué se almacena en la base de datos
            try 
            {
                // Crear una orden de prueba temporal para verificar almacenamiento
                ...
                System.Diagnostics.Debug.WriteLine($"GenerarNumeroOrden: TEST - Insertado orden test con ID={testId}");
                
                // Inmediatamente leer de vuelta para verificar
                var testRecuperado = _dao.ObtenerPorId(testId);
                System.Diagnostics.Debug.WriteLine($"GenerarNumeroOrden: TEST - Recuperado numero_orden={testRecuperado?.NumeroOrden}");
                
                // Limpiar la orden de prueba
                _dao.CambiarEstado(testId, "ANULADA");
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"GenerarNumeroOrden: ERROR en test - {ex.Message}");
            }
        ...
        public ActionResult Detalles(int id)
        {
            ...
            // Cargar lista de bancos para el dropdown
            try
            {
                ViewBag.Bancos = _bancoDao.ObtenerBancos();
            }
            catch
            {
                ViewBag.Bancos = new List<BancoP9>();
            }
