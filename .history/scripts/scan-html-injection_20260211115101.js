...
const excludePatterns = ['Content/plugins', 'Content/dist', 'Content/assets', 'Content/js', '.min.js', '.map'];
  ...
  for (const e of entries) {
    ...
    patterns.forEach(p => {
      ...
      while ((m = re.exec(text)) !== null) {
        const idx = m.index;
        const snippet = text.substr(Math.max(0, idx - 160), Math.min(320, text.length - Math.max(0, idx - 160)));
        // Quick heuristic: require a HTML tag character sequence in the snippet for it to be relevant
        const looksLikeHtml = /<\s*\/?\s*[a-zA-Z]/.test(snippet);
        if (!looksLikeHtml) {
          // If there are no HTML-like tags in the nearby snippet, treat as safe (likely textual template/console log)
          if (p.name === 'template-with-html-and-interp' || p.name === 'string-concat-html-var' || p.name === 'html-template-concat') {
            continue;
          }
        }
        // If snippet contains one of known safe escape functions or pattern that uses text(), skip it
        const safeIndicators = ['AOCR.escapeHtml', 'AOCR.escapeAttr', 'escapeHtml(', 'escapeAttr(', 'HttpUtility.HtmlEncode', 'Html.Encode(', 'HtmlEncode(', '.text(', '.textContent', 'append(AOCR.createOption', '<option', '\/\* safe-template \*\/'];
        const isSafe = safeIndicators.some(function(si){ return snippet.indexOf(si) >= 0; });
        if (!isSafe) {
          safeCount++; // Count only non-safe matches
          // Debug: print detailed info for specific file to investigate remaining matches
          if (full.toLowerCase().includes('formularioemisionaocr')) {
            console.log('\n[debug-scan] file:', full, '\n  pattern:', p.name, '\n  match:', m[0].slice(0,200), '\n  snippet:\n', snippet);
          }
        }
