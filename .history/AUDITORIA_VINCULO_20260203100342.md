...
| 2 | ⚠️ **DOCUMENTACIÓN:** Scripts SQL en `scripts/sql/` están desactualizados - no reflejan la estructura real de la base de datos | P3 |
...
**Columnas que NO existen en la base de datos real:**
- ❌ `para_nombre` - Solo existe en modelo C# pero nunca se usa
- ❌ `es_html` - Solo existe en modelo C# pero nunca se usa
- ❌ `adjunto_*` - Solo existen en modelo C# pero nunca se usan
- ❌ `intentos` / `max_intentos` - Solo existen en modelo C# pero nunca se usan
- ❌ `ultimo_error` - Solo existe en modelo C# pero nunca se usa
- ❌ `fecha_envio` - Solo existe en modelo C# pero nunca se usa
- ❌ `correlation_id` / `numero_orden` / `tipo_notificacion` - Solo existen en modelo C# pero nunca se usan
...
### 3.2 Discrepancia entre Scripts SQL vs Base de Datos Real

**IMPORTANTE:** Los scripts SQL en `scripts/sql/` NO reflejan la estructura real de la base de datos.
...
| `scripts/sql/email_pdf_tables.sql` | `para` | `estado` | `fecha_creacion` | ⚠️ DESACTUALIZADO |
| `scripts/sql/create_email_queue.sql` | `para` | `estado` | `fecha_creacion` | ⚠️ DESACTUALIZADO |
| `scripts/create_email_queue_table.sql` | `para` | `estado` | `fecha_creacion` | ⚠️ DESACTUALIZADO |

**CONCLUSIÓN:** El código C# está CORRECTO y coincide con la base de datos real. Los scripts SQL en `scripts/` están desactualizados y deben ignorarse o actualizarse para reflejar la estructura real.
...
| # | Archivo | Qué hacer | Verificación |
|---|---------|-----------|--------------|
| P1.1 | `scripts/sql/*.sql` | Actualizar scripts desactualizados para usar nombres en inglés que coincidan con la base de datos real | Ejecutar scripts y comparar con `\d email_queue` |
| P1.2 | `CapaDatos/Entidades/EmailQueueItem.cs` | Eliminar propiedades no utilizadas: `ParaNombre`, `EsHtml`, `Adjunto*`, `Intentos`, `MaxIntentos`, `UltimoError`, `FechaEnvio`, `CorrelationId`, `NumeroOrden`, `TipoNotificacion` | Verificar que no se usan en el código |
...
- Los scripts SQL en `scripts/sql/` están desactualizados (usan español) pero no se usan en producción
- Propiedades no utilizadas en `EmailQueueItem` que podrían eliminarse para limpieza de código
- Nomenclatura de `solicitud_id` podría mejorarse a `orden_id` para claridad semántica
