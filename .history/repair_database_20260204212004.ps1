...
try {
    ...
    # Dividir el SQL en comandos individuales (escapar correctamente el regex)
    $comandos = $sqlContent -split "(?<=;)\s*`n" | Where-Object { $_.Trim() -ne "" -and -not $_.Trim().StartsWith("--") }
...
"@
        ...
        $verifyCmd = New-Object Npgsql.NpgsqlCommand($sqlVerify, $connectionObj)
