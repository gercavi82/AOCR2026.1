...
namespace CapaDatos.Services
{
    ...
    public class IdempotencyService : Infrastructure.BaseDAO
    {
        ...
        private void EnsureSchema()
        {
            ...
            lock (_schemaLock)
            {
                ...
                try
                {
                    ...
                    {
                        var sql = @"
                            CREATE TABLE IF NOT EXISTS aocr_idempotency_key (
                            ...
                            CREATE INDEX IF NOT EXISTS idx_idempotency_clave ON aocr_idempotency_key(clave);
                            CREATE INDEX IF NOT EXISTS idx_idempotency_expiracion ON aocr_idempotency_key(fecha_expiracion);
                            CREATE INDEX IF NOT EXISTS idx_idempotency_orden ON aocr_idempotency_key(orden_id);";
                    ...
                    _schemaEnsured = true;
                    _logger.LogInfo("IdempotencyService: Tabla aocr_idempotency_key verificada/creada.");
                }
                catch (Exception ex)
                {
                    _logger.LogWarning("IdempotencyService: Error al crear tabla: " + ex.Message);
                }
            }
        }
        ...
        /// <summary>
        /// Intenta adquirir un lock para la operación (overload simplificado).
        /// Retorna true si se adquirió el lock (operación nueva).
        /// </summary>
        public bool TryAcquire(string clave, string operacion)
        {
            string dummy;
            return TryAcquire(clave, operacion, null, null, out dummy);
        }
