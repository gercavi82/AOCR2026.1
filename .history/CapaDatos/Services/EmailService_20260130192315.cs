...
namespace CapaDatos.Services
{
    ...
    public class QueuedEmailService : IEmailService
    {
        ...
        public void EnviarFacturaGenerada(object orden, byte[] pdfBytes)
        {
            _logger.LogInfo("EnviarFacturaGenerada stub ejecutado", new LogContext { ErrorCode = "EMAIL_FACTURA" });
        }

        public void EnviarNotificacionRechazo(object orden, string motivo)
        {
            _logger.LogInfo("EnviarNotificacionRechazo stub ejecutado", new LogContext { ErrorCode = "EMAIL_RECHAZO", AdditionalData = new System.Collections.Generic.Dictionary<string, object> { ["Motivo"] = motivo } });
        }
    }
}
