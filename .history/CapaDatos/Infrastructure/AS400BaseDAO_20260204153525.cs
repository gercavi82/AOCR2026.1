...
namespace CapaDatos.Infrastructure
{
    ...
    public abstract class AS400BaseDAO : IDisposable
    {
        ...
        private string BuildConnectionString(AS400Credentials creds)
        {
            ...
            // Probar diferentes drivers AS400 disponibles
            // Primer intento: iSeries Access ODBC Driver (detectado en el sistema)
            var connectionString = string.Format(
                "Driver={{iSeries Access ODBC Driver}};System={0};Database={1};Uid={2};Pwd={3};" +
            ...
            System.Diagnostics.Debug.WriteLine($"AS400 Connection String (sin password): {connectionString.Replace($"Pwd={creds.Password}", "Pwd=****")}");
        ...
        private string BuildAlternativeConnectionString(AS400Credentials creds)
        {
            return string.Format(
                "Driver={{Client Access ODBC Driver (32-bit)}};System={0};Database={1};Uid={2};Pwd={3};" +
                "Connection Timeout={4};Query Timeout={5};",
                creds.Server,
                creds.Database ?? creds.Library,
                creds.UserId,
