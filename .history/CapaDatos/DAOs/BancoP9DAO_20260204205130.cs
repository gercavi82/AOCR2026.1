...
namespace CapaDatos.DAOs
{
    ...
    public class BancoP9DAO : AS400BaseDAO
    {
        ...
        public List<BancoP9> ObtenerBancos()
        {
            ...
            try
            {
                // Intentar primero obtener bancos configurables desde parámetros
                var bancosConfigurables = ObtenerBancosConfigurables();
                if (bancosConfigurables.Any())
                {
                    return bancosConfigurables;
                }
                
                // Si no hay parámetros configurados, intentar P9 como fallback
                return bancos;
            ...
            catch (Exception ex)
            {
                ...
                // Return configurable banks as fallback
                if (bancos.Count == 0)
                {
                    bancos = ObtenerBancosConfigurables();
                    
                    // Si aún no hay bancos configurados, usar mínimos por defecto
                    if (bancos.Count == 0)
                    {
                        bancos.Add(new BancoP9 { Codigo = "999", Descripcion = "NO CONFIGURADO - Contacte Administrador" });
                    }
                }
            }
        ...
        /// <summary>
        /// Obtiene la lista de bancos desde parámetros configurables
        /// Reemplaza los valores "quemados" por configuración dinámica
        /// </summary>
        /// <returns>Lista de bancos configurables</returns>
        private List<BancoP9> ObtenerBancosConfigurables()
        {
            var bancos = new List<BancoP9>();
            ...
            try
            {
                ...
                var bancosConfig = parametroDao.ObtenerPorClavePattern("BANCO_");
                
                foreach (var param in bancosConfig)
                {
                    var partes = param.Valor?.Split('|');
                    if (partes != null && partes.Length >= 2)
                    {
                        var codigo = param.Clave.Replace("BANCO_", "");
                        var descripcion = partes[0].Trim();
                ...
                // Ordenar por código
                bancos = bancos.OrderBy(b => b.Codigo).ToList();
            }
            catch (Exception ex)
            {
                System.Diagnostics.Debug.WriteLine($"Error cargando bancos configurables: {ex.Message}");
            }
