...
namespace CapaDatos.DAOs
{
    ...
    public class OrdenRecaudacionDAO : BaseDAO, IOrdenRecaudacionRepository
    {
        ...
        // Compatibilidad sincrónica usada por OrdenController
        public OrdenRecaudacion ObtenerPorId(int id)
        {
            // Change: if (orden.Id == id) to proper comparison
            return ObtenerOrdenPorId(id);
        }

        /// <summary>
        /// Devuelve las órdenes del usuario en forma de DataTable (compatibilidad con vistas antiguas).
        /// </summary>
        public DataTable ObtenerOrdenesPorUsuario(int idUsuario)
        {
            var lista = ListarPorUsuario(idUsuario, null);
            var dt = new DataTable();
            dt.Columns.Add("id", typeof(int));
            dt.Columns.Add("numero_orden", typeof(string));
            dt.Columns.Add("fecha_creacion", typeof(DateTime));
            dt.Columns.Add("estado", typeof(string));
            dt.Columns.Add("subtotal", typeof(decimal));
            dt.Columns.Add("admin", typeof(decimal));
            ...
            foreach (var o in lista)
            {
                dt.Rows.Add(
                    o.Id,
                    o.NumeroOrden ?? string.Empty,
                    o.FechaCreacion,
                    o.Estado ?? string.Empty,
                    o.Subtotal,
                    o.Admin,
                    o.Total,
                    o.Observaciones ?? o.Observacion ?? string.Empty
                );
            }
            return dt;
        }
        ...
        // Compatibilidad: permitir filtro opcional por estado (usado por FinancieroController)
        public List<OrdenRecaudacion> ObtenerTodasLasOrdenes(string estado)
        {
            var lista = ObtenerTodosAsync().Result.ToList();
            if (!string.IsNullOrWhiteSpace(estado))
            {
                var estadoNorm = estado.Trim();
                lista = lista
                    .Where(o => string.Equals(o.Estado, estadoNorm, StringComparison.OrdinalIgnoreCase))
                    .ToList();
            }
            return lista;
        }

        public bool CambiarEstadoOrden(int id, string estado) => ActualizarEstadoAsync(id, estado, "SYSTEM").Result;

        public bool CambiarEstadoOrden(int id, string estado, out string error)
        {
            error = null;
            try { return ActualizarEstadoAsync(id, estado, "SYSTEM").Result; }
        ...
        public List<OrdenRecaudacion> ListarPorUsuario(int idUsuario, string estado)
        {
            var todas = ObtenerTodosAsync().Result.ToList();
        ...
        public bool RegistrarPago(int codigoSolicitud, CapaModelo.PagoModel pago, out string error)
        {
            error = null;
            return true;
        }
        ...
        // Compatibilidad con capa de presentación/financiero
        public CapaDatos.Entidades.Pago ObtenerUltimoPagoPorOrden(int ordenId)
        {
            // TODO: implementar consulta real; por ahora retorna null para evitar fallos de compilación.
            return null;
        }

        public bool ActualizarUltimoPagoEstado(int ordenId, string nuevoEstado, string usuario, string motivo)
        {
            // TODO: implementar actualización real de pagos.
            return true;
        }

        public object ObtenerDatosParaPdf(int ordenId, int pagoId)
        {
            // TODO: mapear DTO real para PdfGeneratorService.
            return null;
        }

        public List<CapaDatos.Models.PagoModel> ObtenerPagosPorOrden(int ordenId)
        {
            // TODO: implementar consulta real
            return new List<CapaDatos.Models.PagoModel>();
        }

        public bool ObtenerEstadisticas(out object estadisticas)
        {
            estadisticas = null;
            // TODO: implementar
            return false;
        ...
        public int ObtenerCodigoSolicitudPorNumero(string numero)
        {
            // TODO: implementar
            return 0;
        }
        ...
        // Estadísticas simples por usuario (stub)
        public Dictionary<string, object> ObtenerEstadisticas(int idUsuario)
        {
            var dict = new Dictionary<string, object>();
            var todas = ObtenerTodosAsync().Result.ToList();
            var propias = todas.Where(o => o.CodigoUsuario == idUsuario).ToList();
            dict["total"] = propias.Count;
            dict["borrador"] = propias.Count(o => string.Equals(o.Estado, "BORRADOR", StringComparison.OrdinalIgnoreCase));
            dict["pendiente"] = propias.Count(o => string.Equals(o.Estado, "PENDIENTE", StringComparison.OrdinalIgnoreCase));
            dict["procesada"] = propias.Count(o => string.Equals(o.Estado, "PROCESADA", StringComparison.OrdinalIgnoreCase));
            dict["facturada"] = propias.Count(o => string.Equals(o.Estado, "FACTURADA", StringComparison.OrdinalIgnoreCase));
            return dict;
        }
        ...
        private OrdenRecaudacion MapearOrden(IDataReader reader)
        {
            ...
            {
                Id = GetInt(reader, "id"),
                CodigoSolicitud = GetString(reader, "numero_orden"), // Reemplaza SolicitudId
                // ContribuyenteId es NotMapped, se omite
                // Iva es NotMapped, se omite
                Observacion = GetString(reader, "observaciones"), // Columna real
                CodigoUsuario = GetString(reader, "usuario_creacion"), // Reemplaza UsuarioCreacion
                // FechaModificacion es NotMapped, se omite
                // UsuarioModificacion es NotMapped, se omite
                // Activo es NotMapped, se omite
                Compania = GetString(reader, "contribuyente_nombre"), // Reemplaza NombreContribuyente
                RucCedula = GetString(reader, "contribuyente_ruc"), // Reemplaza RucContribuyente
                Correo = GetString(reader, "contribuyente_correo"), // Reemplaza EmailContribuyente
