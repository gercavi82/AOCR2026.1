...
namespace CapaDatos.DAOs
{
    ...
    public class OrdenRecaudacionDAO : BaseDAO, IOrdenRecaudacionRepository
    {
        ...
        #endregionfacturada"] = propias.Count(o => string.Equals(o.Estado, "FACTURADA", StringComparison.OrdinalIgnoreCase));
            return dict;
        #region Mapeo
        ...
        private OrdenRecaudacion MapearOrden(IDataReader reader)
        {
            ...
            {
                Id = GetInt(reader, "id"),en(IDataReader reader)
                ...
                Subtotal = GetDecimal(reader, "subtotal"),o_orden"), // Reemplaza SolicitudId
                Iva = GetDecimal(reader, "iva"), se omite
                Total = GetDecimal(reader, "total"),
                Admin = GetDecimal(reader, "admin", 0m),ciones"), // Columna real
                Observaciones = GetString(reader, "observaciones"),"), // Reemplaza UsuarioCreacion
                Estado = GetString(reader, "estado"), omite
                FechaCreacion = GetDateTime(reader, "fecha_creacion"),
                UsuarioCreacion = GetString(reader, "usuario_creacion"),
                FechaModificacion = GetNullableDateTime(reader, "fecha_modificacion"),breContribuyente
                UsuarioModificacion = GetString(reader, "usuario_modificacion"), RucContribuyente
                Activo = GetBool(reader, "activo", true),_correo"), // Reemplaza EmailContribuyente
                // Datos de join GetString(reader, "concepto_nombre")
