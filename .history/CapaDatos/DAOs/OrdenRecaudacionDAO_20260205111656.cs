...
namespace CapaDatos.DAOs
{
    ...
    public class OrdenRecaudacionDAO : IOrdenRecaudacionRepository, IOrdenRecaudacionDAO
    {
        ...
        /// <summary>
        /// Obtiene órdenes por código de solicitud
        /// </summary>
        public List<OrdenRecaudacion> ObtenerPorCodigoSolicitud(int codigoSolicitud)
        {
            var ordenes = new List<OrdenRecaudacion>();
            ...
            {
                _logger.LogError(ex, "Error en ObtenerPorCodigoSolicitud con codigoSolicitud: {codigoSolicitud}", codigoSolicitud);
                throw;
            }
        ...
        /// <summary>
        /// Obtiene una orden específica por código de solicitud
        /// </summary>
        public OrdenRecaudacion ObtenerPrimaPorCodigoSolicitud(int codigoSolicitud)
        {
            try
            {
                var ordenes = ObtenerPorCodigoSolicitud(codigoSolicitud);
                return ordenes?.FirstOrDefault();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error en ObtenerPrimaPorCodigoSolicitud con codigoSolicitud: {codigoSolicitud}", codigoSolicitud);
                return null;
            }
        }
        ...
        /// <summary>
        /// Obtiene el valor de la columna banco (o alternativas) de forma segura.
        /// </summary>
        private string GetSafeBanco(IDataReader reader)
        {
            var columnasBanco = new[] { "banco", "banco_origen", "banco_pagos", "banco_origen_pago" };
            foreach (var columna in columnasBanco)
            {
                try
                {
                ...
                {
                    // Ruta: columna no disponible, seguir probando otras alternativas
                    continue;
                }
            }

            // Ninguna columna explícita existe: inferir desde el método
            System.Diagnostics.Debug.WriteLine("GetSafeBanco: columna explícita no encontrada, se infiere desde método de pago");
            try
            {
        ...
        private string InferirBancoDesdeMetodoPago(string metodoPago)
        {
            ...
            var metodo = metodoPago.ToUpperInvariant().Trim();
            
            // Mapeo específico para códigos de método de pago comunes
            switch (metodo)
            {
                case "D":
                ...
                case "SPI":
                    return "SISTEMA PAGOS INTERBANCARIOS";
                case "PA":
                    return "PAGO EN AGENCIA";
                default:
                    // Si es un texto más largo, buscar palabras clave
