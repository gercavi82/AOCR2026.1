...
namespace CapaModelo.DTOs
{
    public class OrdenRecaudacionPdfDto
    {
        ...
        /// Elimina valores hardcodeados como 8% administrativo
        /// </summary>
        public void CalcularTotales()
        {
            ...
            else
            {
                ...
                {
                    // Fallback con cálculo por defecto basado en configuración alternativa
                    try
                    {
                        // Intentar obtener desde configuración alternativa usando ParametroDAO
                        var parametroDAO = new ParametroDAO();
                        var parametroGastosAdmin = parametroDAO.ObtenerParametroPorCodigo("CALCULO_PORCENTAJE_GASTOS_ADMIN");
                        var porcentajeDefault = parametroGastosAdmin != null ? parametroGastosAdmin.ValorParametro : 8.0m;
                        
                        Subtotal = Math.Round(ValorBase + ValorInspecciones + ValorViaticos, 2);
                        ValorGastosAdmin = Math.Round(ValorViaticos * (porcentajeDefault / 100m), 2);
                        Total = Math.Round(ValorBase + ValorInspecciones + ValorViaticos + ValorGastosAdmin, 2);
                    }
                    catch
                    {
                        // Último fallback - usar mínimo posible
                        Subtotal = Math.Round(ValorBase + ValorInspecciones + ValorViaticos, 2);
                        ValorGastosAdmin = 0m;
                        Total = Math.Round(ValorBase + ValorInspecciones + ValorViaticos + ValorGastosAdmin, 2);
                    }
