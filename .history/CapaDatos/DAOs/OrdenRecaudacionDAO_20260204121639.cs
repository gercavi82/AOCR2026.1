    ...
    public class OrdenRecaudacionDAO : IOrdenRecaudacionRepository, IOrdenRecaudacionDAO
    {
        ...
        public Task<int> ObtenerConsecutivoDiarioAsync(DateTime fecha)
        {
            ...
            try
            {
                ...
                {
                    conn.Open();
                    // Contar órdenes del día actual más específicamente
                    var sql = @"
                        SELECT COALESCE(MAX(
                            CASE 
                                WHEN numero_orden ~ '^OR-[0-9]{14}-[0-9]+$' 
                                THEN CAST(SPLIT_PART(numero_orden, '-', 3) AS INTEGER)
                                ELSE 0
                            END
                        ), 0) as max_consecutivo
                        FROM aocr_or_orden 
                        WHERE fecha_creacion::date = @fecha
                          AND numero_orden LIKE 'OR-' || @fechaStr || '-%'";

                    using (var cmd = new NpgsqlCommand(sql, conn))
                    {
                        cmd.Parameters.AddWithValue("@fecha", fecha.Date);
                        cmd.Parameters.AddWithValue("@fechaStr", fecha.ToString("yyyyMMdd"));
