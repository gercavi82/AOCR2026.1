    ...
    public class PagoDAO : BaseDAO, IPagoRepository
    {
        ...
        public async Task<IEnumerable<Pago>> ObtenerPorEstadoAsync(string estado)
        {
            const string sql = @"
                ...
                FROM aocr_tbpago p
                INNER JOIN aocr_or_orden o ON p.orden_id = o.id
        ...
        public PagoDAO() : this(GetDefaultConnectionString()) { }
        ...
        public Pago ObtenerUltimoPorSolicitud(int solicitudId) => ObtenerPorOrdenIdAsync(solicitudId).Result;
        
        public int Insertar(Pago pago) => CrearAsync(pago).Result;

        // Compatibilidad: insertar con usuario (no usado en stub)
        public int Insertar(Pago pago, string usuario)
        {
        ...
        // Compatibilidad: obtener pago por código de solicitud (vía orden_id)
        public Pago ObtenerPorSolicitud(int codigoSolicitud)
        {
            // Si la relación orden_id -> solicitud existe en la tabla ordenes, aquí debería hacerse un JOIN.
            // Se retorna null para no romper compilación y dejar TODO explícito.
