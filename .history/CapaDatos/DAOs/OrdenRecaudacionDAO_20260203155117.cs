        ...
        public int Insertar(OrdenRecaudacion orden)
        {
            ...
            using (var conn = new NpgsqlConnection(_connectionString))
            {
                ...
                using (var trans = conn.BeginTransaction())
                {
                    try
                    {
                        // Convertir string a int para codigo_usuario y codigo_solicitud
                        var parametros = new
                        {
                            CodigoUsuario = int.TryParse(orden.CodigoUsuario, out int cu) ? cu : (int?)null,
                            CodigoSolicitud = int.TryParse(orden.CodigoSolicitud, out int cs) ? cs : (int?)null,
                            orden.NumeroOrden,
                            orden.FechaCreacion,
                            orden.Estado,
                            orden.Compania,
