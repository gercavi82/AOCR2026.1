    ...
    public class InspeccionDAO
    {
        ...
        public Inspeccion ObtenerPorId(int id)
        {
            const string sql = @"
                SELECT 
                    ...
                    fecha_programada, hora_programmada, duracion_estimada,
                    lugar, latitud, longitud, tipo,
                    observaciones_generales, comentarios, hallazgos_principales,
                    estado, resultado, informe_pdf,
        ...
        public int Crear(Inspeccion i)
        {
            const string sql = @"
                ...
                (codigo_solicitud, codigo_inspector, fecha_programada, hora_programmada, duracion_estimada,
                 lugar, latitud, longitud, tipo, observaciones_generales, comentarios, hallazgos_principales,
                 estado, resultado, ruta_informe, created_at, created_by, updated_at, updated_by)
                VALUES
                (@sol, @insp, @fp, @hp, @dur,
                 @lug, @lat, @lon, @tipo, @obs, @com, @hall,
                 @estado, @res, @ruta, NOW(), @cby, NOW(), @uby)
            ...
            using (var cmd = new NpgsqlCommand(sql, cn))
            {
                ...
                cmd.Parameters.AddWithValue("@dur", (object)i.DuracionEstimada ?? DBNull.Value);
        ...
        public bool GuardarInforme(int id, string rutaInforme, int updatedBy)
        {
            const string sql = @"
                UPDATE public.aocr_tbinspeccion
                SET informe_pdf = @ruta, updated_at = NOW(), updated_by = @uby
                WHERE codigo_inspeccion = @id;";
        ...
        private Inspeccion Map(IDataRecord dr)
        {
            ...
            return new Inspeccion
            {
                ...
                HoraProgramada = dr["hora_programmada"] != DBNull.Value ? (TimeSpan?)dr["hora_programmada"] : null,
                DuracionEstimada = dr["duracion_estimada"] != DBNull.Value ? (int?)Convert.ToInt32(dr["duracion_estimada"]) : null,
                ...
                RutaInforme = dr["informe_pdf"] != DBNull.Value ? dr["informe_pdf"].ToString() : null,
