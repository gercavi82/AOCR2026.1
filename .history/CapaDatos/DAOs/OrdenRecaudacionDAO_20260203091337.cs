...
namespace CapaDatos.DAOs
{
    ...
    public class OrdenRecaudacionDAO : IOrdenRecaudacionRepository, IOrdenRecaudacionDAO
    {
        ...
        {
            int nuevoId = 0;
            _logger.LogDebug("=== DAO Insertar ===");
            _logger.LogDebug("Compania: " + orden.Compania);
            ...
            try
            {
                ...
                {
                    conn.Open();
                    _logger.LogDebug("Conexion abierta");
                    ...
                    using (var cmd = new NpgsqlCommand(sql, conn))
                    {
                        ...
                        _logger.LogDebug("CodigoUsuario recibido: '{0}' (tipo: {1})", orden.CodigoUsuario, orden.CodigoUsuario?.GetType().Name ?? "null");
                        ...
                        {
                            _logger.LogDebug($"CodigoUsuario parseado correctamente: {codigoUsuarioInt}");
                            cmd.Parameters.AddWithValue("@codigo_usuario", codigoUsuarioInt);
                        }
                        else
                        {
                            _logger.LogDebug($"ERROR: No se pudo parsear CodigoUsuario. Valor: '{orden.CodigoUsuario}'");
                        ...
                        _logger.LogDebug("INSERT exitoso, ID: " + nuevoId);
                    }
            ...
            {
                _logger.LogDebug("Insertar ERROR: " + ex.Message);
