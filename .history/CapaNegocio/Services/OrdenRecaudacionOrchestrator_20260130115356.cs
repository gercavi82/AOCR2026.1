...
namespace CapaNegocio.Services
{
    ...
    public class OrdenRecaudacionOrchestrator : IOrdenRecaudacionOrchestrator
    {
            ...
            try
            {
                ...
                if (request.Aprobado)
                {
                    ...
                    try
                    {
                        ...
                        {
                            var notifRequest = new EnviarNotificacionRequest
                            {
                                OrdenId = pago.OrdenId,
                                TipoNotificacion = "PAGO_VALIDADO",
                                EmailDestino = orden.EmailContribuyente,
                                NombreDestino = orden.NombreContribuyente,
                                AdjuntarPdf = false
                            };

                            var notifResult = await EnviarNotificacionAsync(notifRequest);
                            response.NotificacionEnviada = notifResult.Success;
                        }
                    }
        ...
        public async Task<OperationResult<EnviarNotificacionResponse>> EnviarNotificacionAsync(EnviarNotificacionRequest request)
        {
            try
            {
                ...
                var response = new EnviarNotificacionResponse
                {
                    ...
                    FechaEnvio = resultado.Success ? DateTime.Now : (DateTime?)null,
                ...
                return resultado.Success
                    ...
                    : OperationResult<EnviarNotificacionResponse>.Fail("Error al enviar notificación: " + resultado.Error, "EMAIL_ERROR");
            }
            catch (Exception ex)
            {
        ...
        private async Task<string> GenerarNumeroOrdenAsync()
        {
            ...
            return string.Format("ORD-{0:yyyyMMdd}-{1:D4}", fecha, consecutivo + 1);
        }
