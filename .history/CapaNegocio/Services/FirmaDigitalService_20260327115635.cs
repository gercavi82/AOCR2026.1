...
namespace CapaNegocio.Services
{
    public class FirmaDigitalService
    {
        ...
        private static Rectangle ObtenerRectanguloFirma(string rolFirmante)
        {
            ...
            {
                return new Rectangle(372f, 560f, 540f, 650f);
            }
        ...
        private static byte[] EstamparBloqueFirmaVisual(byte[] pdfBytes, string contenidoQr, string nombreFirmante, string rolFirmante, DateTime fechaFirma)
        {
            ...
            using (var output = new MemoryStream())
            {
                using (var stamper = new PdfStamper(reader, output))
                {
                    ...
                    var esAocr = string.Equals((rolFirmante ?? string.Empty).Trim(), "AOCR_FIRMANTE", StringComparison.OrdinalIgnoreCase);
                    var tituloFont = new Font(baseNormal, esAocr ? 6.5f : 9f, Font.NORMAL, BaseColor.BLACK);
                    var nombreFont = new Font(baseBold, esAocr ? 8.8f : 16f, Font.BOLD, BaseColor.BLACK);
                    var detalleFont = new Font(baseNormal, esAocr ? 5.8f : 8.5f, Font.NORMAL, BaseColor.BLACK);

                    var ct = new ColumnText(canvas);
                    ct.SetSimpleColumn(rectTexto.Left, rectTexto.Bottom, rectTexto.Right, rectTexto.Top, esAocr ? 7.4f : 12f, Element.ALIGN_LEFT);
                    ct.AddText(new Phrase(esAocr ? "FIRMA AUTORIZACION\n" : "Firmado electronicamente por:\n", tituloFont));
