...
namespace CapaNegocio.Services
{
    public class FacturacionAS400Service
    {
            ...
            out string mensaje)
        {
            ...
            // --- Idempotency check ---
            var claveIdempotencia = IdempotencyService.GenerarClaveFr3(ordenId, pagoId ?? 0, total);
            ...
            try
            {
                ...
                if (orden == null)
                {
                    ...
                    _syncLog.FallarOperacion(syncLogId, mensaje, false);
                    return false;
                }
                ...
                if (ok)
                {
                    ...
                    _audit.RegistrarFr3Generado(ordenId, fr3Numero, usuario);
                ...
                _syncLog.FallarOperacion(syncLogId, mensaje, true);
                _idempotency.Liberar(claveIdempotencia);
                _audit.RegistrarFr3Error(ordenId, mensaje, usuario);
