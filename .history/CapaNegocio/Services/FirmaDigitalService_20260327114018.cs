        ...
        private static byte[] EstamparBloqueFirmaVisual(byte[] pdfBytes, string contenidoQr, string nombreFirmante, string rolFirmante, DateTime fechaFirma)
        {
            ...
            using (var output = new MemoryStream())
            {
                using (var stamper = new PdfStamper(reader, output))
                {
                    ...
                    var tituloFont = new Font(baseNormal, 9f, Font.NORMAL, BaseColor.BLACK);
                    var nombreFont = new Font(baseBold, 16f, Font.BOLD, BaseColor.BLACK);
                    var detalleFont = new Font(baseNormal, 8.5f, Font.NORMAL, BaseColor.BLACK);

                    var ct = new ColumnText(canvas);
                    ct.SetSimpleColumn(rectTexto.Left, rectTexto.Bottom, rectTexto.Right, rectTexto.Top, 12f, Element.ALIGN_LEFT);
                    ct.AddText(new Phrase("Firmado electronicamente por:\n", tituloFont));
