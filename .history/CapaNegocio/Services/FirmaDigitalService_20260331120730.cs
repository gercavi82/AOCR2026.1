...
namespace CapaNegocio.Services
{
    public class FirmaDigitalService
    {
        ...
        private static Rectangle ObtenerRectanguloFirma(string rolFirmante)
        {
            ...
            {
                return new Rectangle(372f, 194f, 542f, 286f);
            }
        ...
        private static Rectangle ObtenerRectanguloQr(string rolFirmante, Rectangle rectanguloFirma = null)
        {
            ...
            if (rol == "AOCR_FIRMANTE")
            {
                ...
                return new Rectangle(totalAocr.Left + 8f, totalAocr.Bottom + 14f, totalAocr.Left + 68f, totalAocr.Bottom + 74f);
            }
        ...
        private static Rectangle ObtenerRectanguloTextoFirma(string rolFirmante, Rectangle rectanguloFirma = null)
        {
            ...
            {
                var totalAocr = rectanguloFirma ?? ObtenerRectanguloFirma(rolFirmante);
                return new Rectangle(totalAocr.Left + 76f, totalAocr.Bottom + 10f, totalAocr.Right - 8f, totalAocr.Top - 8f);
            }
        ...
        private static byte[] EstamparBloqueFirmaVisual(byte[] pdfBytes, string contenidoQr, string nombreFirmante, string rolFirmante, DateTime fechaFirma, PosicionFirmaVisualPdf posicionFirmaVisual)
        {
            ...
            using (var output = new MemoryStream())
            {
                using (var stamper = new PdfStamper(reader, output))
                {
                    ...
                    var tituloFont = new Font(baseNormal, esAocr ? 6.2f : (esInformeTecnico ? 5.2f : 9f), Font.NORMAL, BaseColor.BLACK);
                    var nombreFont = new Font(baseBold, esAocr ? 8.1f : (esInformeTecnico ? 8.8f : 16f), Font.BOLD, BaseColor.BLACK);
                    ...
                    ct.SetSimpleColumn(rectTexto.Left, rectTexto.Bottom, rectTexto.Right, rectTexto.Top, esAocr ? 6.4f : (esInformeTecnico ? 6.3f : 12f), Element.ALIGN_LEFT);
        ...
        private static Rectangle ObtenerRectanguloFirmaPersonalizado(PdfReader reader, int numeroPagina, string rolFirmante, PosicionFirmaVisualPdf posicionFirmaVisual)
        {
            if (reader != null && posicionFirmaVisual != null && posicionFirmaVisual.EsValida)
            {
                ...
                var width = Limitar(areaWidth * posicionFirmaVisual.AnchoRatio, 72f, areaWidth * 0.45f);
                var height = Limitar(areaHeight * posicionFirmaVisual.AltoRatio, 36f, areaHeight * 0.22f);
                var left = Limitar(areaUtil.Left + (areaWidth * posicionFirmaVisual.PosicionXRatio), areaUtil.Left, areaUtil.Right - width);
                var top = Limitar(areaUtil.Top - (areaHeight * posicionFirmaVisual.PosicionYRatio), areaUtil.Bottom + height, areaUtil.Top);
                var bottom = top - height;
                return new Rectangle(left, bottom, left + width, top);
            }
