...
namespace CapaNegocio.Helpers
{
    ...
    public static class PdfBrandingHelper
    {
        ...
        private static PdfBrandingAssets ResolveAssetsInternal(Func<string, string> mapPath, string source)
        {
            ...
            try
            {
                if (string.IsNullOrWhiteSpace(headerDataUri) && !string.IsNullOrWhiteSpace(headerPhysicalPath) && File.Exists(headerPhysicalPath))
                {
                    headerDataUri = "file:///" + headerPhysicalPath.Replace('\\', '/');
                    LogError("ResolveAssetsInternal", $"Usando fallback file:// para header: {headerDataUri}");
                }

                if (string.IsNullOrWhiteSpace(footerDataUri) && !string.IsNullOrWhiteSpace(footerPhysicalPath) && File.Exists(footerPhysicalPath))
                {
                    footerDataUri = "file:///" + footerPhysicalPath.Replace('\\', '/');
