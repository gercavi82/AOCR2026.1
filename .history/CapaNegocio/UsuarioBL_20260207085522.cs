...
namespace CapaNegocio
{
    public static class UsuarioBL
    {
        ...
        // ================================
        // 6. Obtener Usuarios por Rol (Genérico)
        // ================================
        public static List<Usuario> ObtenerPorRol(string nombreRol)
        {
            try
            {
                return UsuarioDAO.ListarPorRol(nombreRol);
            }
            catch (Exception ex)
            {
                LogBL.RegistrarError($"Error al obtener usuarios por rol {nombreRol}", ex.ToString(), "UsuarioBL");
                return new List<Usuario>();
            }
        }
