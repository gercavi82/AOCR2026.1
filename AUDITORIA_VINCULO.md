...
- **P1:** En el JSON de `FormularioCompleto`, se usan propiedades de `SolicitudAOCR` que no existen en el esquema real (`ObservacionesGenerales`, `TipoSolicitud`, `Ciudad`, etc.). Se perderán datos o fallará si el DAO intenta persistirlos. (CapaPresentacion/Views/SolicitudAOCR/_FormularioEmisionAOCR.cshtml:1182-1197; CapaDatos/DAOs/SolicitudAOCRDAO.cs:100-117)
...
- **P2:** JavaScript con cierre extra `};` puede romper ejecución de scripts y causar “Unexpected token '}'”. (CapaPresentacion/Views/SolicitudAOCR/_FormularioEmisionAOCR.cshtml:946-948)
- **P2:** Uso de operador de propagación nula `?.` en Razor con C# 5 produce error de compilación. (CapaPresentacion/Views/SolicitudAOCR/_FormularioEmisionAOCR.cshtml:1177-1178)
...
| Tabla DB | Columna DB (snake_case) | Tipo DB | Propiedad C# encontrada | Tipo C# | Evidencia | Estado | Corrección sugerida |
|---|---|---|---|---|---|---|---|
...
| aocr_tbsolicitud | tipo_solicitud | (NO EXISTE) | SolicitudAOCR.TipoSolicitud | int? | CapaModelo/SolicitudAOCR.cs:11 | ERROR CRÍTICO | Quitar columna en DAO o agregar en BD |
| aocr_tbsolicitud | ciudad/provincia/pais | (NO EXISTE) | SolicitudAOCR.Ciudad/Provincia/Pais | string | CapaModelo/SolicitudAOCR.cs:21-23 | ERROR CRÍTICO | Quitar del SQL/DAO o agregar en BD |
| aocr_tbsolicitud | codigo_tecnico/created_at/... | (NO EXISTE) | SolicitudAOCR.CodigoTecnico/CreatedAt/... | varios | CapaModelo/SolicitudAOCR.cs:33-44 | ERROR CRÍTICO | Quitar del SQL/DAO o agregar en BD |
...
| aocr_or_orden | codigo_usuario | integer | OrdenRecaudacion.CodigoUsuario | string | CapaDatos/Entidades/OrdenRecaudacion.cs:27-29 | Riesgo | Cambiar a int o usar conversión consistente |
| aocr_or_orden | codigo_solicitud | integer | OrdenRecaudacion.CodigoSolicitud | string | CapaDatos/Entidades/OrdenRecaudacion.cs:31-33 | Riesgo | Cambiar a int o usar conversión consistente |
...
| aocr_or_orden | observacion | (NO EXISTE) | OrdenRecaudacion.Observacion | string | CapaDatos/Entidades/OrdenRecaudacion.cs:46-48 | ERROR CRÍTICO | Agregar columna en BD o eliminar en DAO |
| aocr_or_orden | subtotal/admin/lugar_emision/correo/telefono/concepto_id | (NO EXISTE) | OrdenRecaudacion.Subtotal/Admin/... | varios | CapaDatos/Entidades/OrdenRecaudacion.cs:50-80 | ERROR CRÍTICO | Agregar columnas en BD o ajustar DAO |
...
| aocr_or_orden_detalle | concepto_codigo/descripcion/porcentaje_admin/subtotal/admin | (NO EXISTE) | DetalleOrden.ConceptoCodigo/Descripcion/PorcentajeAdmin/... | varios | CapaDatos/Entidades/DetalleOrden.cs:23-59 | ERROR CRÍTICO | Agregar columnas o ajustar DAO |
...
| aocr_tbdocumento | nombre_archivo | varchar(255) | Documento.NombreArchivo (usado por reflexión) | string | CapaPresentacion/Controllers/SolicitudAOCRController.cs (ProcesarArchivos) | Riesgo | Verificar modelo/DAO |
| aocr_tbdocumento | ruta_guardada | text | Documento.RutaGuardada/RutaArchivo (usado por reflexión) | string | CapaPresentacion/Controllers/SolicitudAOCRController.cs (ProcesarArchivos) | Riesgo | Verificar modelo/DAO |
| aocr_tbdocumento | estado | varchar(20) | Documento.Estado (usado por reflexión) | string | CapaPresentacion/Controllers/SolicitudAOCRController.cs (ProcesarArchivos) | Riesgo | Verificar modelo/DAO |
...
- **Dapper MatchNamesWithUnderscores** está activo, por lo que `snake_case` debería mapear a `PascalCase`. (CapaPresentacion/Global.asax.cs:22-23)
- **Errores críticos de nombres**: los DAOs usan columnas/tablas que **no existen** en el script SQL entregado:
...
|---|---|---|---|---|---|---|
| aocr_or_orden.codigo_usuario | integer | OrdenRecaudacion.CodigoUsuario | string | CapaDatos/Entidades/OrdenRecaudacion.cs:27-29 | Riesgo | Conversión manual, puede guardar NULL |
| aocr_or_orden.codigo_solicitud | integer | OrdenRecaudacion.CodigoSolicitud | string | CapaDatos/Entidades/OrdenRecaudacion.cs:31-33 | Riesgo | Conversión manual, posible pérdida |
| aocr_or_orden.total | numeric(18,2) | OrdenRecaudacion.Total | decimal? | CapaDatos/Entidades/OrdenRecaudacion.cs:56-57 | OK | - |
| aocr_or_orden_detalle.cantidad | integer | DetalleOrden.Cantidad | int | CapaDatos/Entidades/DetalleOrden.cs:35-36 | OK | - |
| aocr_or_orden_detalle.valor_unitario | numeric(18,2) | DetalleOrden.ValorUnitario | decimal | CapaDatos/Entidades/DetalleOrden.cs:38-39 | OK | - |
| aocr_tbsolicitud.codigo_usuario | integer | SolicitudAOCR.CodigoUsuario | int | CapaModelo/SolicitudAOCR.cs:32 | OK | - |
...
- Envia JSON con `contentType: application/json` y URL `FormularioCompleto`. (líneas 1211-1216)
- Incluye `Solicitud`, `Banco`, `NumeroComprobante`, `Aeronaves`. (líneas 1182-1202)

**Controller:** `SolicitudAOCRController.FormularioCompleto(SolicitudAOCRViewModel vm)`
- Recibe el modelo esperado sin `[FromBody]`. En MVC5 el JSON puede bindearse si está habilitado el JSON Value Provider.

**ViewModel:** `CapaPresentacion/Models/SolicitudAOCRViewModel.cs`
- Propiedades coinciden: `Solicitud`, `Aeronaves`, `Banco`, `NumeroComprobante`. (líneas 7-19)

**Estado:** OK **si** JSONValueProvider está habilitado. Si fue removido, fallará el binding.

**Pendiente:** falta el modelo `AeronaveSolicitud` para verificar el mapeo de propiedades (`Fabricante`, `Modelo`, `Matricula`, etc.).

## 5.2 Orden de Recaudación (Form POST)
...
- **aocr_tbsolicitud:** `SolicitudAOCRDAO.InsertarConReturn` usa `RETURNING codigo_solicitud`, pero incluye columnas inexistentes (`tipo_solicitud`, `ciudad`, `created_at`, etc.). La inserción fallará antes de retornar el ID. (CapaDatos/DAOs/SolicitudAOCRDAO.cs:100-117)
...
- **email_queue:** `EmailQueueService` sí coincide con el esquema entregado y retorna `id`. (CapaDatos/Services/EmailQueueService.cs:93-110)
...
- Línea 946-948:
  ...
  ```
  Esto cierra de más el bloque de `$(document).ready`, puede generar `Unexpected token '}'`.

## 7.2 Operador de propagación nula en Razor (C# 5)
**Archivo:** `CapaPresentacion/Views/SolicitudAOCR/_FormularioEmisionAOCR.cshtml` líneas 1177-1178:
```csharp
...
1) `CapaDatos/DAOs/OrdenRecaudacionDAO.cs` – Ajustar SQL de `aocr_or_orden` a columnas reales (id, codigo_usuario, codigo_solicitud, numero_orden, fecha_creacion, estado, compania, ruc_cedula, total).
   - Verificación: crear orden y confirmar que INSERT/SELECT no arroja “column does not exist”.
2) `CapaDatos/DAOs/OrdenRecaudacionDAO.cs` – Ajustar SQL de `aocr_or_orden_detalle` al esquema real (id, orden_id, concepto_id, concepto_nombre, cantidad, valor_unitario, total_linea).
   - Verificación: insertar detalle y leerlo sin excepción.
3) `CapaDatos/DAOs/SolicitudDAO.cs` y `CapaDatos/DAOs/SolicitudAOCRDAO.cs` – Eliminar columnas inexistentes de SELECT/INSERT/UPDATE o migrar BD para incluirlas.
   - Verificación: listar/crear solicitud sin error de columna.
4) `CapaDatos/DAOs/PagoDAO.cs` – Cambiar tablas `pagos`/`ordenes_recaudacion` a las reales (probablemente `aocr_tbpago`/`aocr_or_orden`).
   - Verificación: registrar pago y listar pendientes.

## P1 (alto riesgo)
5) `CapaDatos/Entidades/OrdenRecaudacion.cs` – Cambiar `CodigoUsuario` y `CodigoSolicitud` a `int?` (o normalizar en DAO). Evitar guardar NULL silencioso. (líneas 27-33)
   - Verificación: insertar orden con usuario válido y revisar que se guarda el ID.
6) `CapaPresentacion/Views/SolicitudAOCR/_FormularioEmisionAOCR.cshtml` – Quitar operador `?.` y reescribir con ternarios C#5.
   - Verificación: compilar sin `CS8026`.

## P2 (estabilidad/JS)
7) `CapaPresentacion/Views/SolicitudAOCR/_FormularioEmisionAOCR.cshtml` – Eliminar el cierre extra `};` (líneas 946-948).
   - Verificación: no aparece `Unexpected token '}'` y JS ejecuta.

---

# Pendientes explícitos (archivos faltantes)
