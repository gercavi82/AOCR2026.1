...
const date = check => check.data('inspection-box').find('input');
...
date(gye).val('2026-09-26').trigger('input');
assert.equal(manager.validateAllAirportDates(), true);
assert.equal(manager.getEstaciones()[0].FechaInspeccion, '2026-09-22');
assert.equal(manager.getEstaciones()[1].FechaInspeccion, '2026-09-26');
manager.prepareSubmission($('form'));
assert.equal($('[name="Estaciones[1].FechaInspeccion"]').val(), '2026-09-26');
gye.prop('checked', false).trigger('change');
assert.equal(date(gye).val(), '');
assert.equal(date(gye).prop('required'), false);
assert.equal(date(gye).prop('disabled'), true);
manager.prepareSubmission($('form'));
assert.equal($('[name="Estaciones[1].FechaInspeccion"]').length, 0);
gye.prop('checked', true).trigger('change');
assert.equal(manager.validateAllAirportDates(), false);
gye.prop('checked', false).trigger('change');
other.prop('checked', true).trigger('change');
date(other).val('2026-09-28');
assert.equal(manager.validateAllAirportDates(), false);
