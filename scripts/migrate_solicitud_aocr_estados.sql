...
SELECT 
    ...
    s.fecha_solicitud,
    s.observaciones,
    COALESCE(u_sol.nombreusuario || ' ' || u_sol.apellidousuario, 'N/A') AS tecnico_solicitante,
    CASE 
...
LEFT JOIN usuario u_sol ON s.codigo_usuario_solicitante = u_sol.idusuario
...
-- Verificar tabla subsanaciones existe
SELECT 
    table_name,
    CASE 
        WHEN table_type = 'BASE TABLE' THEN 'OK - Tabla creada'
        ELSE 'ERROR'
    END AS status
FROM information_schema.tables
WHERE table_name = 'aocr_tbsubsanacion';
...
/*
SIGUIENTES PASOS:
1. [COMPLETADO] Ejecutar este script en ambiente de desarrollo
2. [ PENDIENTE ] Actualizar EntityFramework DbContext con nuevas columnas
3. [ PENDIENTE ] Crear DAO para SubsanacionDAO en CapaDatos
4. [ PENDIENTE ] Crear clase de negocio SubsanacionBL en CapaNegocio
5. [ PENDIENTE ] Actualizar SolicitudAOCRController con acciones de subsanacion
6. [ PENDIENTE ] Crear vistas Razor para solicitar/completar subsanaciones
