...
namespace CapaDatos.DAOs
{
    public class SolicitudAOCRDAO
    {
        ...
        // ✅ COMPATIBILIDAD
        public SolicitudAOCR ObtenerPorCodigo(int codigo) => ObtenerPorId(codigo);
...
(
  numero_solicitud, fecha_solicitud, tipo_solicitud, estado,
  nombre_operador, ruc, razon_social, email, telefono, direccion, ciudad, provincia, pais,
...
VALUES
(
  @numero, COALESCE(@fecha, NOW()), @tipo_solicitud, COALESCE(@estado,'BORRADOR'),
  @nombre_operador, @ruc, @razon_social, @email, @telefono, @direccion, @ciudad, @provincia, @pais,
  @representante_legal, @cedula_representante,
  @tipo_operacion, @descripcion_operacion, @observaciones,
  @codigo_usuario, @codigo_tecnico,
...
RETURNING codigo_solicitud;";
                ...
                using (var cmd = new NpgsqlCommand(sql, cn))
                {
                    ...
                    cmd.Parameters.AddWithValue("@estado", (object)(s.Estado ?? "BORRADOR"));
                    ...
                    cmd.Parameters.AddWithValue("@codigo_usuario", s.CodigoUsuario);
                    cmd.Parameters.AddWithValue("@codigo_tecnico", (object)s.CodigoTecnico ?? DBNull.Value);

                    cmd.Parameters.AddWithValue("@created_by", (object)(s.CreatedBy ?? "sistema"));
